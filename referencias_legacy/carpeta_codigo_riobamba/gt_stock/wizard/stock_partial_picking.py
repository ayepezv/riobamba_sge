...
class stockPartialLine(osv.TransientModel):
...
class stock_partial_picking(osv.osv_memory):
    ...
    def do_partial(self, cr, uid, ids, context=None):
        ...
        picking = stock_picking.browse(cr, uid, partial.picking_id.id)
    ...
    def _validate_picking(self, cr, uid, ids,context=None):
        ...
        partial = self.browse(cr, uid, ids[0], context=context)
        picking = picking_obj.browse(cr, uid, context['active_id'])
        if not (uid in [p.id for p in picking.bodega_id.usuario_ids] or uid==picking.bodega_id.responsable_id.id):
            raise osv.except_osv(('Error de permisos'), ('Usted no tiene permisos para realizar movimientos en esta bodega'))
        #validar el tipo tambien
        return True
