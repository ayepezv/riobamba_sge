...
Comparecen a la celebración del presente contrato individual de trabajo a plazo indefinido, por una parte el <b>${user.company_id.name or ''|entity}</b>, representado legalmente por sus personeros: Ingeniero Marcos Alberto Chica Cárdenas; y, Abogado Felipe Dau Ochoa, en sus calidades de ALCALDE Y PROCURADOR SÍNDICO MUNICIPAL, respectivamente en su orden, a quien para efecto y resultado del presente instrumento se lo denominará como <b>“EL CONTRATANTE”</b> o <b>“EL EMPLEADOR”</b>; y por otra parte comparece el señor <b>${o.employee_id.complete_name}</b>, portador de la cédula de ciudadanía Nº. <b>${o.employee_id.name}</b>, por sus propios derechos, capaz ante la Ley, a quien en adelante para los mismos efectos se llamará como <b>“EL TRABAJADOR”</b>. Los comparecientes son mayores de edad, hábiles para obligarse, contratar y ejercer esta clase de actos, con capacidad civil amplia y suficiente cual en derecho se requiere.
<P>
<b>SEGUNDA: ANTECEDENTES.-</b>
El <b>${user.company_id.name or ''|entity}</b> para el cumplimiento de sus actividades y desarrollo de las tareas propias de su actividad necesita contratar los servicios laborales para satisfacer exigencias de la municipalidad originadas por ${o.employee_id.department_id.name}.

En vista de que el empleado ${o.employee_id.complete_name} fue contratado por El Empleador, y desde el ${o.date_start} viene prestando sus servicios en la entidad municipal, se establece por esta circunstancia el presente contrato a plazo indefinido.

El señor ${o.employee_id.complete_name} declara tener los conocimientos necesarios para el desempeño del trabajo indicado, por lo que en base a las consideraciones anteriores y por lo expresado en los numerales siguientes, el EMPLEADOR y el TRABAJADOR proceden a celebrar el presente contrato individual de trabajo a plazo indefinido.
<P>
<b>TERCERA: OBJETO DEL CONTRATO.-</b>
${o.employee_id.complete_name} se compromete con el empleador a prestar sus servicios lícitos y personales a órdenes y bajo dependencia patronal del Gobierno Autónomo Descentralizado Municipal del cantón  Naranjal, y desempeñará las funciones de ${o.job_id.name} (Departamento de ${o.employee_id.department_id.name}), para lo cual declara tener los conocimientos y capacidades necesarias, sus acciones serán supervisadas por el Director de ${o.employee_id.department_id.name}.
<P>
<BR>
...
El lugar principal de trabajo del contratado se establece en el Palacio Municipal en las Oficinas del Departamento de ${o.employee_id.department_id.name}, ubicado en el Edificio Municipal situado en ${user.company_id.street or ''|entity}  de la cabecera cantonal de ${user.company_id.city or ''|entity}, provincia de/del ${user.company_id.state_id.name or ''|entity}.
<P>
<BR>
...
El ${user.company_id.name or ''|entity} se compromete pagar al trabajador previa presentación de informes mensuales aprobados por el Jefe de Área, el valor de (${o.wage}), como remuneración mensual unificada, con cargo a la partida presupuestaria Nº. ${o.budget_id.code}.
<P>
