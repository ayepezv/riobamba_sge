...
class audittrail_objects_proxy(object_proxy):
    ...
    def get_value_text(self, cr, uid, pool, resource_pool, method, field, value):
        ...
        pre_field = (resource_pool._all_columns.get(field))
        if pre_field == None:
            return value
        else:
            field_obj = pre_field.column
...
class gtAudittrail(osv.Model):
  _inherit = "audittrail.rule"

  def create_rules_mass(self, cr, uid, ids, context=None):
    rule_obj = self.pool.get('audittrail.rule')
    rule_ids = rule_obj.search(cr, uid, [])
    rule_obj.subscribe(cr, uid, rule_ids)
    return True
