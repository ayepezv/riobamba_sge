...
class extraRecaudacionLine(osv.Model):
    _name = 'extra.recaudacion.line'
    _columns = dict(
        o_id = fields.many2one('extra.recaudacion','Recaudacion'),
        name = fields.many2one('account.account','Cuenta'),
        monto = fields.float('Monto'),
    )
extraRecaudacionLine()
class extraDeposito(osv.Model):
    _name = 'extra.deposito'
    _columns = dict(
        o_id = fields.many2one('extra.recaudacion','Recaudacion'),
...
extraDeposito()
class extraRecaudacion(osv.Model):
    _name = 'extra.recaudacion'
    _order = 'date desc'
    _columns = dict(
        name = fields.char('Descripcion',size=256,required=True),
        date = fields.date('Fecha'),
        banco = fields.many2one('account.journal','Banco'),
        banco_id = fields.many2one('account.account','Banco'),
        line_ids = fields.one2many('extra.recaudacion.line','o_id','Detalle Recaudacion'),
        deposito_ids = fields.one2many('extra.deposito','o_id','Detalle Depositos'),
        move_id = fields.many2one('account.move','Comprobante Contable'),
    )

    def generarExtra(self, cr, uid, ids, context=None):
        certificate_line_obj = self.pool.get('budget.certificate.line')
        account_obj = self.pool.get('account.account')
        move_obj = self.pool.get('account.move')
        extra_obj = self.pool.get('extra.recaudacion')
        ...
        for this in self.browse(cr, uid, ids):
            ...
            extra_obj.write(cr, uid, this.id, {
                'move_id':move_id,
            })
            ...
            for line in this.line_ids:
                ...
                #buscar el post y el item de la partida
                account = account_obj.browse(cr, uid, line.name.id)
                item_ids = item_obj.search(cr, uid, [('budget_post_id','=',account.budget_id.id),('date_start','<=',this.date),('date_end','>=',this.date)])
                if item_ids:
                    ...
                    move_id,account_id,credit,journal_id,period_id,state,company_id,currency_id,date,budget_id_cert,budget_id,budget_post,name,migrado,budget_paid) VALUES (%s,%s, %s, %s, %s, %s, %s,%s,%s,%s,%s,%s,%s,%s,%s)''',(move_id,line.name.id,line.monto,move.journal_id.id,move.period_id.id,state_aux,company_aux,currency_aux,date_aux,certificate_id,b_id,p_id,name_aux,False,True))
                else:
                    cr.execute('''
                    INSERT INTO account_move_line (
                    move_id,account_id,credit,journal_id,period_id,state,company_id,currency_id,date,name,migrado) VALUES (%s,%s, %s, %s, %s, %s, %s,%s,%s,%s,%s)''',(move_id,account.id,line.monto,move.journal_id.id,move.period_id.id,state_aux,company_aux,currency_aux,date_aux,name_aux,False))
            for line_deposito in this.deposito_ids:
                cr.execute('''
                INSERT INTO account_move_line (
                move_id,account_id,debit,journal_id,period_id,state,company_id,currency_id,date,name,migrado) VALUES (%s,%s, %s, %s, %s, %s, %s,%s,%s,%s,%s)''',(move_id,this.banco_id.id,line_deposito.monto,move.journal_id.id,move.period_id.id,state_aux,company_aux,currency_aux,date_aux,line_deposito.name,False))
        return True

extraRecaudacion()

class descuentoCobroLine(osv.Model):
    _name = 'descuento.cobro.line'
    _columns = dict(
        c_id = fields.many2one('descuento.cobro','Descuento'),
        account_id = fields.many2one('account.account','Cuenta'),
        monto = fields.float('Valor'),
    )
descuentoCobroLine()

class descuentoCobro(osv.Model):
    _name = 'descuento.cobro'
    _order = 'date desc'
    ...
    _columns = dict(
        ...
        date = fields.date('Fecha'),
        move_id = fields.many2one('account.move','Comprobante Contable'),
        budget_id = fields.many2one('budget.post','Partida DESCUENTO(560106)'),
        line_ids = fields.one2many('descuento.cobro.line','c_id','Detalle Cobro'),
    )

    def generarDescuento(self, cr, uid, ids, context=None):
        certificate_line_obj = self.pool.get('budget.certificate.line')
        certificate_obj = self.pool.get('budget.certificate')
        account_obj = self.pool.get('account.account')
        move_obj = self.pool.get('account.move')
        move_line_obj = self.pool.get('account.move.line')
        descuento_obj = self.pool.get('descuento.cobro')
        project_obj = self.pool.get('project.project')
        item_obj = self.pool.get('budget.item')
        ...
        for this in self.browse(cr, uid, ids):
            ...
            descuento_obj.write(cr, uid, this.id, {
                'move_id':move_id,
            })
            move = move_obj.browse(cr, uid, move_id)
            aux_total = 0
            certificate_ing_ids = certificate_obj.search(cr, uid, [('tipo_aux','=','ingreso'),('date_commited','<=',this.date),
                                                                   ('date_confirmed','>=',this.date)],limit=1)
            ...
            for line in this.line_ids:
                aux_total += line.monto
                #tambien va con la partida
                #aqui pregunta si es 6 hace esto caso contrario si es 11311va por el else
                if line.account_id.code[0:1]=='6':
                    account_ids = account_obj.search(cr, uid, [('account_rec_id','=',line.account_id.id)])
                    if account_ids:
                        account_aux = account_obj.browse(cr, uid, account_ids[0])
                        item_ids = item_obj.search(cr, uid, [('budget_post_id','=',account_aux.budget_id.id),('date_start','<=',this.date),('date_end','>=',this.date)],limit=1)
                        if not item_ids:
                            raise osv.except_osv(('Error Configuracion !'),
                                                 ("No existe la partida '%s'") % (line.budget_id.code))
                    else:
                        item_ids = item_obj.search(cr, uid, [('budget_post_id','=',line.account_id.budget_id.id),('date_start','<=',this.date),('date_end','>=',this.date)],limit=1)
                        if not item_ids:
                            raise osv.except_osv(('Error Configuracion !'),
                                                 ("No existe la partida '%s'") % (line.account_id.budget_id.code))
                        account_aux = line.account_id.account_rec_id
                    certificate_line_ids = certificate_line_obj.search(cr, uid, [('budget_id','=',item_ids[0])])
                    if certificate_line_ids:
                        move_line_obj.create(cr, uid, {
                            'move_id':move_id,
                            'account_id':account_aux.id,
                            'credit':line.monto,
                            'budget_id_cert':certificate_line_ids[0],
                            'budget_paid':True,
                        })
                else:
                    item_ids = item_obj.search(cr, uid, [('budget_post_id','=',line.account_id.budget_id.id),
                                                         ('date_start','<=',this.date),('date_end','>=',this.date)],limit=1)
                    if not item_ids:
                        raise osv.except_osv(('Error Configuracion !'),
                                             ("No existe la partida '%s'") % (line.budget_id.code))
                    certificate_line_ids = certificate_line_obj.search(cr, uid, [('budget_id','=',item_ids[0])])
            ...
            #antes agrupaba ahora hace por cada linea, se identa uno hacia adentro
                item_ids = item_obj.search(cr, uid, [('budget_post_id','=',this.budget_id.id),('date_start','<=',this.date),('date_end','>=',this.date)],limit=1)
                if not item_ids:
                    raise osv.except_osv(('Error Configuracion !'),
                                         ("No existe la partida '%s'") % (line.budget_id.code))
            #migrated para descuento, partida 56
                ...
                itemm_anterior = itemm_obj.search(cr, uid, [('program_code','=',item_descuento.program_id.sequence),
                                                          ('commited_amount','=',aux_total),('date','=',this.date)])
                context_unlink = {}
...
class bajaLine(osv.Model):
    ...
    _columns = dict(
        ...
        amount_anterior = fields.float('Valor Anterior'),
        amount = fields.float('Valor Actual'),
    )
bajaLine()
...
class baja(osv.Model):
    ...
    def generarBaja(self, cr, uid, ids, context=None):
        ...
        for this in self.browse(cr, uid, ids):
            journal_ids = journal_obj.search(cr, uid, [('name','=','INGRESOS')],limit=1)
            if not journal_ids:
                raise osv.except_osv('Error configuracion','No esta creada un diario de ingresos.')
            period_ids = period_obj.find(cr, uid, this.date)
            if not period_ids:
                raise osv.except_osv('Error configuracion','No existe periodo para la fecha de comprobante.')
            journal_id = journal_ids[0]
                ...
                if account_ids:
                    for account_id in account_ids:
                        ...
                        if account.account_rec_id or account.account_pay_id:
                            ...
                            break
                if not band:
                    post_ids_padre = post_obj.search(cr, uid, [('code','=',line.budget_id.budget_post_id.code[0:6])])
                    if post_ids_padre:
                        account_ids = account_obj.search(cr, uid, [('budget_id','=',post_ids_padre)])
                        if account_ids:
                            for account_id in account_ids:
                                account = account_obj.browse(cr, uid, account_ids[0])
                                if account.account_rec_id or account.account_pay_id:
...
#                cxp = account.account_rec_id.id
                monto = (line.amount)*(-1)
                monto_aux = line.amount_anterior
                #entrada cxc debe
                if monto!=0:
                    cr.execute('''
                    INSERT INTO account_move_line (
                ...
                #anios anteriores
                cbajaaa_ids = parameter_obj.search(cr, uid, [('key','=','cbajaaa')],limit=1)
                cpatbaja_ids = parameter_obj.search(cr, uid, [('key','=','cpatbaja')],limit=1)
                if cbajaaa_ids:
                    cbajaaa = parameter_obj.browse(cr, uid, cbajaaa_ids[0]).value
                    account_ids = account_obj.search(cr, uid, [('code','=',cbajaaa)],limit=1)
                    if account_ids:
                        cxp = account_ids[0]
                if cpatbaja_ids:
                    cpatbaja = parameter_obj.browse(cr, uid, cpatbaja_ids[0]).value
                    account_ids = account_obj.search(cr, uid, [('code','=',cpatbaja)],limit=1)
                    if account_ids:
                        account = account_obj.browse(cr, uid, account_ids[0])
...
class emisionLine(osv.Model):
    ...
    _columns = dict(
        ...
        budget_id = fields.many2one('budget.item','Partida/Rubro'),
        account_id = fields.many2one('account.account','Cuenta Contable Ingreso'),
        monto = fields.float('Valor'),
    )
emisionLine()
class emision(osv.Model):
    ...
    def generarEmision(self, cr, uid, ids, context=None):
        ...
        for this in self.browse(cr, uid, ids):
            ...
            aux_total = 0
            for line in this.line_ids:
                if not (line.account_id or line.budget_id):
                    raise osv.except_osv('Error de usuario', 'Verifique que la linas tengan al menos cuenta o partida')
                project_ids = project_obj.search(cr, uid, [('type_budget','=','ingreso')],limit=1)
                project = project_obj.browse(cr, uid, project_ids[0])
                if line.budget_id:
                    certificate_id = certificate_line_obj.create(cr, uid, {
                    ...
                    account_ids = account_obj.search(cr, uid, [('budget_id','=',line.budget_id.budget_post_id.id)])
                    if not account_ids:
                        aux_code_base = line.budget_id.budget_post_id.code[0:6]
                        post_base_ids = post_obj.search(cr, uid, [('code','=',aux_code_base)])
                        account_ids = account_obj.search(cr, uid, [('budget_id','=',post_base_ids[0])])
                        if not account_ids:
                            aux_texto = line.budget_id.budget_post_id.code + ' - ' + line.budget_id.budget_post_id.name
                            raise osv.except_osv(('Error de configuracion'),('La partida % seleccionada no tiene cuenta asociada') %(aux_texto))
                    pat = False
                    for account_id in account_ids:
                        account = account_obj.browse(cr, uid, account_id)
                        if account.account_rec_id:
                    ...
                    if not pat:
                        aux_texto = line.budget_id.budget_post_id.code + ' - ' + line.budget_id.budget_post_id.name
                        raise osv.except_osv(('Error de configuracion'),('La partida % seleccionada no tiene cuenta de ingreso asociada') %(aux_texto))
                else:
                    #buscar el post y el item de la partida
                    account = account_obj.browse(cr, uid, line.account_id.id)
                    if not account.budget_id:
                        aux_texto = account.code + ' - ' + account.name
                        raise osv.except_osv(('Error de configuracion'),('NO hay partida presupuestaria asociada a la cuenta %s')%(aux_texto))
                    item_ids = item_obj.search(cr, uid, [('budget_post_id','=',account.budget_id.id),('date_start','<=',this.date),('date_end','>=',this.date)])
                    if not item_ids:
                        aux_texto = account.budget_id.code
                        raise osv.except_osv(('Error de configuracion'),('NO hay item para la partida %s')%(aux_texto))
                    certificate_id = certificate_line_obj.create(cr, uid, {
                        'project_id':project.id,
                        'task_id':project.tasks[0].id,
                        'budget_id':item_ids[0],
                    ...
                    p_id = certificate.budget_post.id
                if not account.account_rec_id:
                    raise osv.except_osv('Error de configuracion', 'La cuenta % seleccionada no tiene cuenta por cobrar asociada' %(account.code))
                cxp = account.account_rec_id.id
                monto = line.monto
                #entrada cxc debe
                cr.execute('''
...
class devolucion(osv.Model):
    ...
    def devolverEfectivo(self, cr, uid, ids, context=None):
        ...
        for this in self.browse(cr, uid, ids):
            ...
            move = move_obj.browse(cr, uid, move_id)
            aux_total = 0
            cert_ing_ids = cert_obj.search(cr,uid, [('tipo_aux','=','ingreso'),('date_confirmed','>=',this.date),('date','<=',this.date)])
            if not cert_ing_ids:
                raise osv.except_osv('Error de configuracion', 'Para devolucion necesita que este creado el presupuesto de ingresos')
            for line in this.line_ids:
                aux_total -= line.monto
                certificate_ids = certificate_line_obj.search(cr, uid, [('budget_post','=',line.budget_id.id),
                                                                        ('certificate_id','=',cert_ing_ids[0])],limit=1)
                if certificate_ids:
        ...
        for this in self.browse(cr, uid, ids):
            ...
            for line in this.line_ids:
                if not (line.account_id or line.budget_id):
                    raise osv.except_osv('Error de usuario', 'Verifique que la linas tengan al menos cuenta o partida')
                aux_total += line.monto
                project_ids = project_obj.search(cr, uid, [('type_budget','=','ingreso'),('date_start','<=',this.date),('date','>=',this.date)],limit=1)
                project = project_obj.browse(cr, uid, project_ids[0])
                if line.budget_id:
                    item_ids = item_obj.search(cr, uid, [('budget_post_id','=',line.budget_id.id),('date_start','<=',this.date),('date_end','>=',this.date)],limit=1)
                    if not item_ids:
                        raise osv.except_osv(('Error Configuracion !'),
                                             ("No existe la partida '%s'") % (line.budget_id.code))
                    certificate_id = certificate_line_obj.create(cr, uid, {
                    ...
                    account_ids = account_obj.search(cr, uid, [('budget_id','=',line.budget_id.id)])
                    if not account_ids:
                        #busco por la de mayor [0:6]
                        aux_code_budget = line.budget_id.code[0:6]
                        post_aux_ids = post_obj.search(cr, uid, [('code','=',aux_code_budget)])
                        account_ids = account_obj.search(cr, uid, [('budget_id','in',post_aux_ids)])
                        if not account_ids:
                            band_aux = False
#                            raise osv.except_osv(('Error de configuracion !'),
#                                                 ("No existe cuenta contable asociada a la partida '%s'")%(line.budget_id.code))
                    ...
                    for account_id in account_ids:
                        ...
                        if account.account_rec_id:
                            band_aux = True
                            break
                    #si no encuentra en partida debe
                    if not band_aux:
                        account_ids = account_obj.search(cr, uid, [('budget_haber_id','=',line.budget_id.id)])
                        if not account_ids:
                            #busco por la de mayor [0:6]
                            aux_code_budget = line.budget_id.code[0:6]
                            post_aux_ids = post_obj.search(cr, uid, [('code','=',aux_code_budget)])
                            account_ids = account_obj.search(cr, uid, [('budget_haber_id','in',post_aux_ids)])
                            if not account_ids:
                                raise osv.except_osv(('Error de configuracion !'),
                                                     ("No existe cuenta contable asociada a la partida '%s'")%(line.budget_id.code))
                        for account_id in account_ids:
                    ...
                    b_id = certificate.budget_id.id
                    p_id = certificate.budget_post.id
                if not account.account_rec_id.id:
                    aux_texto = account.code
                    raise osv.except_osv(('Error de configuracion'),('NO esta configurada la cuneta de cobro en %s')%(aux_texto))
                if account.account_pay_id:
                    cxp = account.account_pay_id.id
                else:
                    cxp = account.account_rec_id.id
            ...
            if aux_total<0:
                cr.execute('''
                INSERT INTO account_move_line (
                move_id,account_id,credit,journal_id,period_id,state,company_id,currency_id,date,name,migrado) VALUES (%s,%s, %s, %s, %s, %s, %s,%s,%s,%s,%s)''',(move_id,banco_account_id,abs(aux_total),move.journal_id.id,move.period_id.id,state_aux,company_aux,currency_aux,date_aux,name_aux,False))
            else:
                cr.execute('''
                INSERT INTO account_move_line (
...
class accountRecaudacionLine(osv.Model):
    ...
    _columns = dict(
        ...
        partida_id = fields.char('Partida',size=32),
        account_id = fields.char('Cuenta Contable',size=64),
...
#        anterior_value = fields.float('Anio Anterior'), #debe ser funcion
        ...
        is_emision = fields.boolean('Con Emision',help="Si marca este campo el sistema hara devengado y pago"),
        )
recaudacionLine()

class postEmision(osv.Model):
...
class recaudacionTercero(osv.Model):
    _name = 'recaudacion.tercero'
    _order = 'name asc'
    _columns = dict(
        name = fields.char('Cuenta',size=64),
        monto = fields.float('Valor'),
        rec_id = fields.many2one('account.recaudacion','Recaudacion'),
    )
recaudacionTercero()

class otrosRecaudacion(osv.Model):
    _name = 'otros.recaudacion'
    _columns = dict(
        otro_id = fields.many2one('account.recaudacion','Recaudacion'),
        account_id = fields.many2one('account.account','Cuenta Contable'),
        monto = fields.float('Valor'),
        tipo = fields.selection([('Debe','Debe'),('Haber','Haber')],'Tipo'),
    )
otrosRecaudacion()
...
#                post_ids = budget_post.search(cr, uid, [('code','=',line_line[0].replace('.',''))])
    ...
    def contabilizar_recaudado(self, cr, uid, ids, context=None):
        ...
        period_obj = self.pool.get('account.period')
        certificate_obj = self.pool.get('budget.certificate')
        #self._get_partida(cr, uid, ids, context)
        for this in self.browse(cr, uid, ids):
            ...
            if this.move_id:
                if this.move_id.state=='draft':
                    cr.execute("delete from account_move_line where move_id=%s"%(this.move_id.id))
                    if this.move_id.name=='/':
                        move_obj.unlink(cr, uid, [this.move_id.id],aux=1)
                        move_id = move_obj.create(cr, uid, {
                            'certificate_id':certificate_ids[0],
                            'ref': this.name,
...
#            certificate_ids = certificate_line_obj.search(cr, uid, [('budget_id','=',tabla.budget_id.id)],limit=1)
            if not certificate_ids:
                raise osv.except_osv(('Error de Configuracion !'),
            ...
            b_id = certificate.budget_id.id
            p_id = certificate.budget_post.id
            # si aux_anterior mayor a cero haga sino no
            if aux_anterior>0:
                cr.execute('''
                INSERT INTO account_move_line (
...
#            raise osv.except_osv('Error de configuracion', 'No existe diario de RECAUDACION por favor cree uno')
    ...
    def _get_partida(self, cr, uid, ids, a,b,c):
        #aqui considerar el anio
        res = {}
        item_obj = self.pool.get('budget.item')
        tabla_obj = self.pool.get('cuenta.anterior')
        tabla_ids = tabla_obj.search(cr, uid, [])
        ...
        for this in self.browse(cr, uid, ids):
            aux_date = this.date
            item_ids = item_obj.search(cr, uid, [('budget_post_id','=',tabla.budget_id.budget_post_id.id),('date_start','<=',aux_date),('date_end','>=',aux_date)])
            if item_ids:
                res[this.id] = item_ids[0]
                return res
            else:
                res[this.id] = tabla.budget_id.id
                return res

    def _get_partida2(self, cr, uid, ids, context=None):
        #aqui considerar el anio
        item_obj = self.pool.get('budget.item')
        tabla_obj = self.pool.get('cuenta.anterior')
        tabla_ids = tabla_obj.search(cr, uid, [])
    ...
    def _get_code_name(self, cr, uid, ids, name, args, context=None):
        res = {}
        for this in self.browse(cr, uid, ids):
            res[this.id] = "Recaudacion - " + this.date
        return res

    _columns = dict(
        tercero_ids = fields.one2many('recaudacion.tercero','rec_id','Fondos Ajenos Detalle'),
        ...
        partida_anterior = fields.function(_get_partida, store=True, string='Partida de cuentas por cobrar anterior',type='many2one',relation='budget.item'),
        ...
        name = fields.function(_get_code_name, store=True, string='Codigo Documento',type='char',size=32),#fields.char('Codigo Documento',size=50),
