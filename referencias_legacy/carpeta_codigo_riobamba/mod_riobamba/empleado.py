...
class empleadoKardex(osv.TransientModel):
   ...
   def exportaKdxEmpleado(self, cr, uid, ids, context=None):
        for this in self.browse(cr, uid, ids):
            writer = XLSWriter.XLSWriter()
            writer.append(['KARDEX DE CONTRATOS DE FUNCIONARIOS'])
            writer.append(['DESDE',this.fecha])
            writer.append(['FUNCIONARIO','CEDULA','CONTRATO','DEPARTAMENTO','CARGO','RMU','FECHA DESDE','FECHA HASTA','ACTIVO'])
            for line in this.line_ids:
               #writer.append([line.employee_id.complete_name,line.employee_id.name])
               for line_line in line.line_ids:
                  writer.append([line.employee_id.complete_name,line.employee_id.name,line_line.contract_id.name,line_line.contract_id.department_id.name or line_line.contract_id.employee_id.department_id.name,line_line.contract_id.job_id.name,line_line.contract_id.wage,line_line.contract_id.date_start,line_line.contract_id.date_end,line_line.estado])
        writer.save("kardexFuncionario.xls")
        out = open("kardexFuncionario.xls","rb").read().encode("base64")
        self.write(cr, uid, ids, {'datas': out, 'datas_fname': 'kardexFuncionario.xls'})
        return True
