...
class rol_alimentacion(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(rol_alimentacion, self).__init__(cr, uid, name, context=context)
...
#        cabecera_todo = []
        ...
        if band:
            ...
            for rol_individual_id in roles_ids:#registro.payroll_id.slip_ids:
                rol_individual = rol_obj.browse(self.cr, self.uid, rol_individual_id)
                aux_key2 = rol_individual.contract_id.program_id.sequence+'.510306'
                aux_key = rol_individual.contract_id.program_id.sequence+'.510306.' + rol_individual.contract_id.employee_id.name#rol_individual.contract_id.budget_ind + rol_individual.contract_id.employee_id.name
...
report_sxw.report_sxw('report.rol_alimentacion',
                       'hr.payslip.run', 
                       'addons/mod_riobamba/report/rol_alimentacion.mako',
                       parser=rol_alimentacion,
                       header=False)
