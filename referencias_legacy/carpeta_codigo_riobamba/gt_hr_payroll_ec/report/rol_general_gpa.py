...
class ctaAnticipoLine(osv.TransientModel):
    _name = 'cta.anticipo.line'
    _columns = dict(
        c_id = fields.many2one('verifica.cta.anticipo','Detalle'),
        employee_id = fields.many2one('hr.employee','Empleado'),
        partner_id = fields.many2one('res.partner','Proveedor'),
        cta = fields.many2one('account.account','Cuenta Anticipo'),
        cta_n = fields.many2one('account.account','Cuenta Anticipo Nueva'),
    )
ctaAnticipoLine()
class verificaCtaAnticipo(osv.TransientModel):
    _name = 'verifica.cta.anticipo'
    _columns = dict(
        rubro_id = fields.many2one('hr.salary.rule','Rubro Anticipo'),
        line_ids = fields.one2many('cta.anticipo.line','c_id','Detalle'),
    )

    def actualizaCta(self, cr, uid, ids, context=None):
        partner_obj = self.pool.get('res.partner')
        for this in self.browse(cr, uid, ids):
            for line in this.line_ids:
                if line.cta_n:
                    if line.partner_id:
                        partner_obj.write(cr, uid, line.partner_id.id,{
                            'anticipo_id':line.cta_n.id,
                        })
        return True


    def verificaCta(self, cr, uid, ids, context=None):
        account_obj = self.pool.get('account.account')
        partner_obj = self.pool.get('res.partner')
        line_obj = self.pool.get('cta.anticipo.line')
        payslip_line_obj = self.pool.get('hr.payslip.line')
        run_obj = self.pool.get('hr.payslip.run')
        slip_obj = self.pool.get('hr.payslip')
        run = run_obj.browse(cr, uid, context['active_id'])
        slip_ids = slip_obj.search(cr, uid, [('payslip_run_id','=',run.id)])
        for this in self.browse(cr, uid, ids):
            lineas_antes = line_obj.search(cr, uid, [('c_id','=',this.id)])
            if lineas_antes:
                line_obj.unlink(cr, uid, lineas_antes)
            lineas_anticipo_ids = payslip_line_obj.search(cr, uid, [('slip_id','in',slip_ids),('salary_rule_id','=',this.rubro_id.id)])
            if lineas_anticipo_ids:
                for linea_anticipo_id in lineas_anticipo_ids:
                    linea_anticipo = payslip_line_obj.browse(cr, uid, linea_anticipo_id)
                    partner_ids = partner_obj.search(cr, uid, [('ced_ruc','=',linea_anticipo.employee_id.name)])
                    if partner_ids:
                        partner = partner_obj.browse(cr, uid, partner_ids[0])
                        if partner.anticipo_id:
                            line_obj.create(cr, uid, {
                                'c_id':this.id,
                                'employee_id':linea_anticipo.employee_id.id,
                                'cta':partner.anticipo_id.id,
...
verificaCtaAnticipo()
class resumen_rol(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(resumen_rol, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'time': time,
            'cr':cr,
            'uid': uid,
        })
report_sxw.report_sxw('report.resumen_rol',
                       'resumen.rol', 
                       'gt_hr_payroll_ec/report/resumen_rol.mako',
                       parser=resumen_rol,
                       header=True)
...
#        cabecera_todo = []
        ...
        if band:
            ...
            roles_ids = rol_obj.search(self.cr, self.uid, [('payslip_run_id','=',payroll.id),('program_id','=',programa.id)])
            roles_all = rol_obj.search(self.cr, self.uid, [('payslip_run_id','=',payroll.id)])
            rubros = self.get_rubros(roles_all)
...
#                    departamentos.append([rol_individual.department_id.id,rol_individual.department_id.name])
                    ...
                    if diccionario.has_key(rol_individual.program_id.id):
                      if diccionario[rol_individual.program_id.id].has_key(rol_individual.employee_id.department_id.id):
                        if diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id].has_key(rol_individual.employee_id.complete_name):
                            #aqui se viene contra la linea del rol y el rubro
                            #preguntar si no es agrpado pase
                            if not rubro.salary_rule_id.agrupar:
                                if diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name].has_key(rubro.salary_rule_id.id):
                                    diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name][rubro.salary_rule_id.id] += rubro.amount
                                else:
                                    diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name][rubro.salary_rule_id.id] = rubro.amount
                            else:
                                ...
                                if diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name].has_key(rubro_aux.id):
                                    diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name][rubro_aux.id] += rubro.amount
                                else:
                                    diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name][rubro_aux.id] = rubro.amount
                        else:
                            diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name] = {'cedula': rol_individual.employee_id.name, 'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}
                      else:
                        diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id] = {rol_individual.employee_id.complete_name: {'cedula': rol_individual.employee_id.name, 'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}}
                    else:
                        diccionario[rol_individual.program_id.id] ={rol_individual.employee_id.department_id.id: {rol_individual.employee_id.complete_name: {'cedula': rol_individual.employee_id.name, 'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}}}
                    ...
                    if rule_ids_tot_ing:
                        ...
                        diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name][rubro_aux_tot_ing.id] = rol_individual.aportable + rol_individual.allowance + rol_individual.basic
                    ...
                    if rule_ids_tot_egr:
                        ...
                        diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name][rubro_aux_tot_egr.id] = rol_individual.deduction
                    ...
                    if rule_ids_total:
                        ...
                        diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name][rubro_aux_total.id] = aux#rol_individual.aportable + rol_individual.allowance + rol_individual.basic - rol_individual.deduction
                #import pdb
                #pdb.set_trace()
                diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name]['dias laborados'] = 0
                for asistencia in rol_individual.worked_days_line_ids:
                    if asistencia.code=='WORK100' or asistencia.code=='VAC' or asistencia.code=='ENF' or asistencia.code=='MAT':
                        diccionario[rol_individual.program_id.id][rol_individual.employee_id.department_id.id][rol_individual.employee_id.complete_name]['dias laborados']+=asistencia.number_of_days
        programa_clean = []
        ...
        writer_dict = []
        cabecera = ['NUM.','FUNCIONARIO: APELLIDOS Y NOMBRES','DIAS LAB.']
        pie = {}
        for rubro in rubros_clean:
            cabecera.append(rubro[2])
        cabecera.append("FIRMA&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")
...
#                pdb.set_trace()
                if ocur_index == 3:
                    ...
                    linea.append(diccionario[programa[0]][departamento[0]][empleado]['dias laborados'])
                    #linea.append(diccionario)
                else:
...
#        self.write(cr, uid, ids, {'datas': out, 'datas_fname': 'resumen_rol.xls'})
    ...
    def generate_dict2(self, payroll,programa):
        rol_obj = self.pool.get('hr.payslip')
        rule_obj = self.pool.get('hr.salary.rule')
        diccionario = {}
        diccionario_totales = {}
...
report_sxw.report_sxw('report.rol_general_gpa',
                       'hr.payslip.run', 
                       'addons/gt_hr_payroll_ec/report/rol_general_gpa.mako',
...
class hr_payroll_export(osv.osv_memory):
    ...
    def generar_archivo_rol(self, cr, uid, ids, context={}):
        ...
        for registro in self.browse(cr, uid, ids, context):
            for rol_individual in registro.payroll_id.slip_ids:
                for rubro in rol_individual.line_ids:
                    ...
                    departamentos.append([rol_individual.department_id.id,rol_individual.department_id.name])
                    #programa tiene el par (id,name)
                    programas.append([rol_individual.program_id.id,rol_individual.program_id.name])
                    
                    if diccionario.has_key(rol_individual.program_id.id):
                    
                      if diccionario[rol_individual.program_id.id].has_key(rol_individual.department_id.id):
                        if diccionario[rol_individual.program_id.id][rol_individual.department_id.id].has_key(rol_individual.employee_id.complete_name):
                            if diccionario[rol_individual.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name].has_key(rubro.salary_rule_id.id):
                                diccionario[rol_individual.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name][rubro.salary_rule_id.id] += rubro.amount
                            else:
                                diccionario[rol_individual.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name][rubro.salary_rule_id.id] = rubro.amount
                        else:
                            diccionario[rol_individual.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name] = {'cedula': rol_individual.employee_id.name, 'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}
                      else:
                        diccionario[rol_individual.program_id.id][rol_individual.department_id.id] = {rol_individual.employee_id.complete_name: {'cedula': rol_individual.employee_id.name, 'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}}
                    else:
                        diccionario[rol_individual.program_id.id] ={rol_individual.department_id.id: {rol_individual.employee_id.complete_name: {'cedula': rol_individual.employee_id.name, 'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}}}
                    #al final del for del rol colocar los totales de ingresos y egresos
                    rule_ids_tot_ing = rule_obj.search(cr, uid, [('name','=','TOTAL INGRESOS')],limit=1)
                    if rule_ids_tot_ing:
                        rubro_aux_tot_ing = rule_obj.browse(cr, uid, rule_ids_tot_ing[0])
                        diccionario[rol_individual.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name][rubro_aux_tot_ing.id] = rol_individual.aportable + rol_individual.allowance + rol_individual.basic
                    rule_ids_tot_egr = rule_obj.search(cr, uid, [('name','=','TOTAL EGRESOS')],limit=1)
                    if rule_ids_tot_egr:
                        rubro_aux_tot_egr = rule_obj.browse(cr, uid, rule_ids_tot_egr[0])
                        diccionario[rol_individual.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name][rubro_aux_tot_egr.id] = rol_individual.deduction
                    rule_ids_total = rule_obj.search(cr, uid, [('name','in',('TOTAL','NETO A RECIBIR'))],limit=1)
                    if rule_ids_total:
                        aux = rol_individual.aportable + rol_individual.allowance + rol_individual.basic - rol_individual.deduction
                        if aux<0.1:
                            aux = round(aux,2)
                        rubro_aux_total = rule_obj.browse(cr, uid, rule_ids_total[0])
                        diccionario[rol_individual.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name][rubro_aux_total.id] = aux#rol_individual.aportable + rol_individual.allowance + rol_individual.basic - rol_individual.deduction
        programa_clean = []
        for key in programas:
            if key not in programa_clean:
                programa_clean.append(key)                
        ...
        for programa in programa_clean:
          for departamento in departamentos_clean:
            if not diccionario[programa[0]].has_key(departamento[0]):
                continue
            linea = [programa[1],departamento[1]]
            writer.append([''])
            writer.append(linea)
            pie = {}
            claves = diccionario[programa[0]][departamento[0]].keys()
            claves.sort()
            for empleado in claves:
                linea = [diccionario[programa[0]][departamento[0]][empleado]['cedula'],empleado,diccionario[programa[0]][departamento[0]][empleado]['partida'],diccionario[programa[0]][departamento[0]][empleado]['tipo cuenta'],diccionario[programa[0]][departamento[0]][empleado]['numero cuenta']]
                for rubro in rubros_clean:
                    if not pie.has_key(rubro[0]):
                        pie.update({rubro[0]:0.00})
                    if diccionario[programa[0]][departamento[0]][empleado].has_key(rubro[0]):
    ...
    def generar_archivo_rol2(self, cr, uid, ids, context={}):
        #diccionario.values() devuelve los valores del diccionario
        #diccionario.keys() devuelve las claves o cabeceras del diccionario
        #diccionario.items() devuelve el par (clave,valor) de cada registro del diccionario
        diccionario = {}
...
#        cabecera_todo = []
        ...
        for registro in self.browse(cr, uid, ids, context):
            for rol_individual in registro.payroll_id.slip_ids:
                rol_individual = rol_obj.browse(cr, uid, rol_individual.id)
                for rubro in rol_individual.line_ids:#payslip lines
                    #rubro tiene la informacion (id,secuencia,name)
                    #rubros.append([rubro.salary_rule_id.id,rubro.salary_rule_id.sequence,rubro.salary_rule_id.name])
                    #departamentos tiene el par (id,name)
                    ...
                    #programa tiene el par (id,name)
                    if diccionario.has_key(rol_individual.contract_id.program_id.id):
                      if diccionario[rol_individual.contract_id.program_id.id].has_key(rol_individual.department_id.id):
                        if diccionario[rol_individual.contract_id.program_id.id][rol_individual.department_id.id].has_key(rol_individual.employee_id.complete_name):
                            #aqui se viene contra la linea del rol y el rubro
                            #preguntar si no es agrpado pase
                            if not rubro.salary_rule_id.agrupar:
                                if diccionario[rol_individual.contract_id.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name].has_key(rubro.salary_rule_id.id):
                                    diccionario[rol_individual.contract_id.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name][rubro.salary_rule_id.id] += rubro.amount
                                else:
                                    diccionario[rol_individual.contract_id.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name][rubro.salary_rule_id.id] = rubro.amount
                            else:
                                #buscar los de agrupacion de ingresos y egresos
                                if rubro.salary_rule_id.category_id.code in ('ING','APT'):
                                    ...
                                    if rule_ids_egr:
                                        rubro_aux = rule_obj.browse(self.cr, self.uid, rule_ids_egr[0])
                                if diccionario[rol_individual.contract_id.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name].has_key(rubro_aux.id):
                                    diccionario[rol_individual.contract_id.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name][rubro_aux.id] += rubro.amount
                                else:
                                    diccionario[rol_individual.contract_id.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name][rubro_aux.id] = rubro.amount
                        else:
                            diccionario[rol_individual.contract_id.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name] = {'cedula': rol_individual.employee_id.name, 'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}
                      else:
                        diccionario[rol_individual.contract_id.program_id.id][rol_individual.department_id.id] = {rol_individual.employee_id.complete_name: {'cedula': rol_individual.employee_id.name, 'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}}
                    else:
                        diccionario[rol_individual.contract_id.program_id.id] ={rol_individual.department_id.id: {rol_individual.employee_id.complete_name: {'cedula': rol_individual.employee_id.name, 'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}}}
                    ...
                    if rule_ids_tot_ing:
                        rubro_aux_tot_ing = rule_obj.browse(cr, uid, rule_ids_tot_ing[0])
                        diccionario[rol_individual.contract_id.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name][rubro_aux_tot_ing.id] = rol_individual.aportable + rol_individual.allowance + rol_individual.basic
                    rule_ids_tot_egr = rule_obj.search(cr, uid, [('name','=','TOTAL EGRESOS')],limit=1)
                    if rule_ids_tot_egr:
                        rubro_aux_tot_egr = rule_obj.browse(cr, uid, rule_ids_tot_egr[0])
                        diccionario[rol_individual.contract_id.program_id.id][rol_individual.department_id.id][rol_individual.employee_id.complete_name][rubro_aux_tot_egr.id] = rol_individual.deduction
        programa_clean = []
        for key in programas:
            if key not in programa_clean:
                programa_clean.append(key)                
...
class rol_extra(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(rol_extra, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'time': time,
            'cr':cr,
            'uid': uid,
    ...
    def get_rubros(self, roles_ids):
        ...
        for rubro_id in self.cr.fetchall(): 
            rubro = rule_obj.browse(self.cr, self.uid, rubro_id[0])
            if rubro.category_id.code in ('COMP'):
                if not rubro.agrupar:
                    if not [rubro.id,rubro.sequence,rubro.name] in rubros_ingreso:
                        rubros_ingreso.append([rubro.id,rubro.sequence,rubro.name])
        return rubros_ingreso + rubros_egreso
    ...
    def get_rubros2(self, roles_ids):
        ...
        for rol_individual_id in roles_ids:#registro.payroll_id.slip_ids:
            ...
            for rubro in rol_individual.line_ids:
                ...
                if rubro.salary_rule_id.category_id.code in ('COMP'):
                    if not rubro.salary_rule_id.agrupar:
...
#        cabecera_todo = []
        ...
        if band:
            ...
            for rol_individual_id in roles_ids:#registro.payroll_id.slip_ids:
                rol_individual = rol_obj.browse(self.cr, self.uid, rol_individual_id)
                if rol_individual.contract_id.budget_ind:
                    aux_key = rol_individual.contract_id.budget_ind + rol_individual.contract_id.employee_id.name
                else:
                    aux_key = rol_individual.contract_id.budget_id.budget_post_id.code + rol_individual.contract_id.employee_id.name
                for rubro in rol_individual.line_ids:#payslip lines
                    if diccionario.has_key(rol_individual.contract_id.program_id.id):
...
#        self.write(cr, uid, ids, {'datas': out, 'datas_fname': 'resumen_rol.xls'})
    ...
    def generate_dictAntes(self, payroll,programa):
        rol_obj = self.pool.get('hr.payslip')
        rule_obj = self.pool.get('hr.salary.rule')
        diccionario = {}
        diccionario_totales = {}
...
report_sxw.report_sxw('report.rol_extra',
                       'hr.payslip.run', 
                       'addons/gt_hr_payroll_ec/report/rol_extra.mako',
                       parser=rol_extra,
                       header=False)


class rol_decimo(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(rol_decimo, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'time': time,
            'cr':cr,
            'uid': uid,
    ...
    def get_rubros(self, roles_ids):
        ...
        for rubro_id in self.cr.fetchall(): 
            rubro = rule_obj.browse(self.cr, self.uid, rubro_id[0])
            if rubro.category_id.code in ('BASIC','ING','APT','COMP'):
                if not rubro.agrupar:
                    if not [rubro.id,rubro.sequence,rubro.name] in rubros_ingreso:
                        rubros_ingreso.append([rubro.id,rubro.sequence,rubro.name])
        return rubros_ingreso + rubros_egreso
    ...
    def get_rubros2(self, roles_ids):
        ...
        for rol_individual_id in roles_ids:#registro.payroll_id.slip_ids:
            ...
            for rubro in rol_individual.line_ids:
                #rubro tiene la informacion (id,secuencia,name) exepto otros ingresos y otros egresos, estos se agregan al final OtrosDescuentos y OING
                if rubro.salary_rule_id.category_id.code in ('ING','BASIC','APT','COMP'):
                    if not rubro.salary_rule_id.agrupar:
                        if not [rubro.salary_rule_id.id,rubro.salary_rule_id.sequence,rubro.salary_rule_id.name] in rubros_ingreso:
                            rubros_ingreso.append([rubro.salary_rule_id.id,rubro.salary_rule_id.sequence,rubro.salary_rule_id.name])
        #si no hay de adem agregar al diccionario
...
#        cabecera_todo = []
                    ...
                    if diccionario.has_key(rol_individual.contract_id.program_id.id):
                      if diccionario[rol_individual.contract_id.program_id.id].has_key(dept_id):
                        if diccionario[rol_individual.contract_id.program_id.id][dept_id].has_key(aux_key):
                            ...
                            diccionario[rol_individual.contract_id.program_id.id][dept_id][aux_key] = {'nombre':rol_individual.employee_id.complete_name,'cedula': rol_individual.employee_id.name,'fecha':rol_individual.contract_id.date_start,'sueldo':rol_individual.contract_id.wage ,'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}
                      else:
                        diccionario[rol_individual.contract_id.program_id.id][dept_id] = {aux_key: {'nombre':rol_individual.employee_id.complete_name,'cedula': rol_individual.employee_id.name,'fecha':rol_individual.contract_id.date_start,'sueldo':rol_individual.contract_id.wage, 'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}}
                    else:
                        diccionario[rol_individual.contract_id.program_id.id] ={dept_id: {aux_key: {'nombre':rol_individual.employee_id.complete_name,'sueldo':rol_individual.contract_id.wage,'fecha':rol_individual.contract_id.date_start,'cedula': rol_individual.employee_id.name, 'tipo cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.type_cta or '-'), 'numero cuenta': (rol_individual.employee_id.bank_account_id and rol_individual.employee_id.bank_account_id.acc_number or '-'), 'partida': (rol_individual.contract_id.budget_ind and rol_individual.contract_id.budget_ind or '-'), rubro.salary_rule_id.id: rubro.amount}}}
        programa_clean = []
        for key in programas:
            if key not in programa_clean:
                programa_clean.append(key)                
        ...
        writer = XLSWriter.XLSWriter()
        writer_dict = []
        cabecera = ['NUM.','FUNCIONARIO: APELLIDOS Y NOMBRES','FECHA CONTRATO','SUELDO']
        pie = {}
        for rubro in rubros_clean:
            cabecera.append(rubro[2])
        #cabecera.append("PARTIDA&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")
...
#                    linea.append(empleado)
                    linea.append(diccionario[programa[0]][departamento[0]][empleado]['nombre'])
                    linea.append(diccionario[programa[0]][departamento[0]][empleado]['fecha'])
                    linea.append(diccionario[programa[0]][departamento[0]][empleado]['sueldo'])
                for rubro in rubros_clean:
#                    j += 1
                    if not pie.has_key(rubro[0]):
                        pie.update({rubro[0]:0.00})
...
report_sxw.report_sxw('report.rol_decimo',
                       'hr.payslip.run', 
                       'addons/gt_hr_payroll_ec/report/rol_decimo.mako',
                       parser=rol_decimo,
                       header=False)



class rol_fr(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(rol_fr, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'time': time,
            'cr':cr,
            'uid': uid,
    ...
    def get_rubros(self, roles_ids):
        ...
        for rubro_id in self.cr.fetchall(): 
            rubro = rule_obj.browse(self.cr, self.uid, rubro_id[0])
            if rubro.category_id.code in ('ING'):
                if not rubro.agrupar:
                    if not [rubro.id,rubro.sequence,rubro.name] in rubros_ingreso:
                        rubros_ingreso.append([rubro.id,rubro.sequence,rubro.name])
        return rubros_ingreso + rubros_egreso
    ...
    def get_rubros2(self, roles_ids):
        ...
        for rol_individual_id in roles_ids:#registro.payroll_id.slip_ids:
            ...
            for rubro in rol_individual.line_ids:
                #rubro tiene la informacion (id,secuencia,name) exepto otros ingresos y otros egresos, estos se agregan al final OtrosDescuentos y OING
                if rubro.salary_rule_id.category_id.code in ('ING'):
                    if not rubro.salary_rule_id.agrupar:
                        if not [rubro.salary_rule_id.id,rubro.salary_rule_id.sequence,rubro.salary_rule_id.name] in rubros_ingreso:
                            rubros_ingreso.append([rubro.salary_rule_id.id,rubro.salary_rule_id.sequence,rubro.salary_rule_id.name])
        #si no hay de adem agregar al diccionario
...
report_sxw.report_sxw('report.rol_fr',
                       'hr.payslip.run', 
                       'addons/gt_hr_payroll_ec/report/rol_fr.mako',
                       parser=rol_fr,
                       header=False)
