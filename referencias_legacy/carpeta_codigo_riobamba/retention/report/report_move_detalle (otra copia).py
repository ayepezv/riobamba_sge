...
class moveDetalle(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(moveDetalle, self).__init__(cr, uid, name, context=context)
        self.sum_debit = 0.00
    ...
    def get_pay_title(self, move):
        ...
        for line in move.line_id:
            if line.credit>0:
                ...
                if journal_ids:
                    letras = self.get_letras_move(line.credit)
                    aux = line.credit
    ...
    def set_context(self, objects, data, ids, report_type=None):
        #Your code goes here
        return super(moveDetalle, self).set_context(objects, data, ids, report_type=report_type)
            ...
            if line.budget_id_cert:
                if not data_lines.get(line.account_id.code):
                    data_lines[line.account_id.code] = {'code': line.account_id.code,
                                           ...
                                           'debit': 0,
                                           'credit': 0,
                                           'budget_id':line.budget_id_cert.budget_id.code,
                                           'seq':line.seq,
                                       }
                if line.debit != 0 and line.credit == 0:     
                    data_lines[line.account_id.code]['debit'] += line.debit    
                else:
                    data_lines[line.account_id.code]['credit'] += line.credit    
            else:
                if not data_lines.get(line.account_id.code):
                    data_lines[line.account_id.code] = {'code': line.account_id.code,
                                           'name': line.account_id.name,
                ...
                if line.debit != 0 and line.credit == 0:     
                    ...
                    data_lines[line.account_id.code]['credit'] += line.credit    
#            if line.debit != 0 and line.credit == 0:     
#                data_lines[line.id]['debit'] = line.debit    
#            else:
#                data_lines[line.id]['credit'] = line.credit    
            self.sum_debit += line.debit
            self.sum_credit += line.credit
        values = data_lines.itervalues()
        for line in values:
            lines_data.append(line)
        newlist = sorted(lines_data, key=itemgetter('seq'))
        return newlist
...
report_sxw.report_sxw('report.account.move.detalle','account.move','addons/retention/report/report_move_detalle.rml',parser=moveDetalle)
