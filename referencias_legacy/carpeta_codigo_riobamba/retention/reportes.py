...
class balancePartidas(osv.TransientModel):
    _name = 'balance.partidas'
    _order = 'partida asc'
    _columns = dict(
        rel_id = fields.many2one('rel.partida.cuenta','Relacion'),
        partida = fields.char('Partida',size=2),
        cuenta = fields.char('Cuenta',size=10),
        debe = fields.float('Debe'),
        haber = fields.float('Haber'),
        devengado = fields.float('Devengado'),
        pagado = fields.float('Pagado/Recaudado'),
        dif_devengado = fields.float('Dif. Devengado'),
        dif_pagado = fields.float('Dif. Pagado'),
    )

balancePartidas()

class relacionCuentas(osv.TransientModel):
    _name = 'relacion.cuentas'
    _columns = dict(
        rel_id = fields.many2one('rel.partida.cuenta','Relacion'),
        partida = fields.char('Partida',size=2),
        cuenta = fields.char('Cuenta',size=10),
...
class relPartidaCuenta(osv.TransientModel):
    _name = 'rel.partida.cuenta'
    _columns = dict(
        ...
        line_ids = fields.one2many('balance.partidas','rel_id','Balance Partidas'),
        line2_ids = fields.one2many('relacion.cuentas','rel_id','Relacion Cuentas'),
    )

    def computeRelCuentas(self, cr, uid, ids, context):
        ...
        line_obj = self.pool.get('balance.partidas')
        partida_cta = {}
        context = {}
        context2 = {}
        for this in self.browse(cr, uid, ids):
            lines_antes = line_obj.search(cr, uid, [('rel_id','=',this.id)])
            if lines_antes:
                line_obj.unlink(cr, uid, lines_antes)
            poa_ids = poa_obj.search(cr, uid, [('date_start','>=',this.date_start),('date_end','<=',this.fiscalyear_id.date_stop)])
            if not poa_ids:
                texto = ustr("No hay presupuesto definido para el periodo fiscal seleccionado")
                raise osv.except_osv('Error', texto)
            context = {'by_date':True,'date_start': this.date_start, 'date_end': this.date_end,'poa_id':poa_ids[0]}            
            context2 = {'fiscalyear':this.fiscalyear_id.id,'state':'posted','date_from': this.date_start,'date_to':this.date_end}
            cta_ids = cta_obj.search(cr, uid, [],order='partida desc')
            if cta_ids:
                for cta_id in cta_ids:
                    cta = cta_obj.browse(cr, uid, cta_id)
                    if not cta.partida in partida_cta:
                        if cta.cta[0:3] in ('213','113'):
                            partida_cta[cta.partida] = cta.cta
            for partida_line in partida_cta:
                aux_devengado = aux_pagado = dif_devengado = dif_pagado = 0 
                item_ids = item_obj.search(cr, uid, [('code_aux','like',partida_line+'%'),('poa_id','=',poa_ids[0])])
                if item_ids:
                    for item in item_obj.browse(cr, uid, item_ids, context):
                        if item.budget_post_id.code[0:2]==partida_line:
                            aux_devengado += item.devengado_amount
                            aux_pagado += item.paid_amount
                account_ids = account_obj.search(cr, uid, [('code','=',partida_cta[partida_line])])
                account = account_obj.read(cr, uid, account_ids[0], ['code','name','debit','credit', 'balance','level'], context2)
                if int(partida_line)>50: #haber con devengado
                    dif_devengado = account['credit'] - aux_devengado
                    dif_pagado = account['debit'] - aux_pagado
                else:
                    dif_devengado = account['debit'] - aux_devengado
                    dif_pagado = account['credit'] - aux_pagado
                line_obj.create(cr, uid, {
                    'rel_id':this.id,
                    'partida':partida_line,
                    'cuenta':account['code'],
                    'debe':account['debit'],
                    'haber':account['credit'],
                    'devengado':aux_devengado,
                    'pagado':aux_pagado,
                    'dif_devengado':dif_devengado,
...
class BalanceCedula(osv.TransientModel):
    ...
    _columns = dict(
        ...
        data_balance_be = fields.binary('Archivo de Balance BEDE', filters="*.txt",readonly=True),
        filename_balance_be = fields.char('Archivo balance BEDE', size=128),
        data_cedulas_bi = fields.binary('Archivo de Cedulas Ingreso BEDE', filters="*.txt",readonly=True),
        filename_cedulas_bi = fields.char('Archivo Cedulas Ingreso BEDE', size=128), 
        data_cedulas_be = fields.binary('Archivo de Cedulas Egreso BEDE', filters="*.txt",readonly=True),
        filename_cedulas_be = fields.char('Archivo Cedulas Egreso BEDE', size=128), 
        data_balance_xls = fields.binary('Archivo de Balance Excel', filters="*.xls",readonly=True),
        filename_balance_xls = fields.char('Archivo balance Excel', size=128),
    )
    ...
    def gen_esigef_files(self, cr, uid, ids, context):
      ...
      total_reform_ing = 0
      total_reform_gas = 0
      mes_1 = datetime.datetime.strptime(formulario.date_end, '%Y-%m-%d').month
      if mes_1<10:
          mes = '0'+str(mes_1)
      else:
          mes = str(mes_1)
      mes_aux = datetime.datetime.strptime(formulario.date_end, '%Y-%m-%d').strftime("%B").upper()+datetime.datetime.strptime(formulario.date_end, '%Y-%m-%d').strftime("%Y").upper()
      for line in dic_ord:
         if line['code']!= '0' and line['code']!= 'total' and line['final'] and line['tipo']=='ingreso':
            ...
            if line['planned_amount']!=0 or line['reform_amount'] != 0 or line['codif_amount']!=0 or line['devengado_amount']!=0 or line['paid_amount']!=0 or line['devengado_balance']!=0:
               ...
               datas_ing_bede.append(['68070000'+line['code'][0:6]+'9',mes,unicodedata.normalize('NFKD',line['general_budget_name']).encode('ascii','ignore'),'%.2f'%line['planned_amount'],'%.2f'%line['reform_amount'],'%.2f'%line['codif_amount'],'%.2f'%line['devengado_amount'],'%.2f'%line['paid_amount']])
      budget_buf_bi = StringIO.StringIO() 
      writer_bi = csv.writer(budget_buf_bi, delimiter='\t')
      writer_bi.writerows(datas_ing_bede)
      out_cedulas_bi = base64.encodestring(budget_buf_bi.getvalue())
      budget_buf_bi.close()
      ...
      for line in dic_ord:
         if line['code']!='0' and line['code']!= 'total' and line['final'] and line['tipo']=='gasto':
            ...
            if line['planned_amount']!=0 or line['reform_amount'] !=0 or line['codif_amount']!=0 or line['commited_amount']!=0 or line['devengado_amount']!=0 or line['paid_amount']!=0 or line['commited_balance']!=0 or line['devengado_balance']:
               ...
               aux_commited_balance = aux_devengado_balance = 0 
               if line['commited_balance'] > 0.01 or line['commited_balance'] < -0.01:
                   aux_commited_balance = line['commited_balance']
               if line['devengado_balance'] > 0.01 or line['devengado_balance'] < -0.01:
                   aux_devengado_balance = line['devengado_balance']
               datas_gas.append([mes,'G',line['code'][0:2],line['code'][2:4],line['code'][4:6],'000',line['orientacion'],'%.2f'%line['planned_amount'],'%.2f'%line['reform_amount'],'%.2f'%line['codif_amount'],'%.2f'%line['commited_amount'],'%.2f'%line['devengado_amount'],'%.2f'%line['paid_amount'],'%.2f'%aux_commited_balance,'%.2f'%aux_devengado_balance])
               datas_gas_bede.append(['6807000099'+line['code'][0:6]+'9',mes,unicodedata.normalize('NFKD',line['general_budget_name']).encode('ascii','ignore'),'%.2f'%line['planned_amount'],'%.2f'%line['reform_amount'],'%.2f'%line['codif_amount'],'%.2f'%line['devengado_amount'],'%.2f'%line['paid_amount']])
      budget_buf_be = StringIO.StringIO() 
      writer_be = csv.writer(budget_buf_be, delimiter='\t')
      writer_be.writerows(datas_gas_bede)
      out_cedulas_be = base64.encodestring(budget_buf_be.getvalue())
      budget_buf_be.close()
      writer.writerows(datas_gas)
      out_cedulas = base64.encodestring(budget_buf.getvalue())
      budget_buf.close()
      filename_cedulas = "ESIGEF CED. INGRESOS-GASTOS-%s.txt"%(mes_aux)
      filename_cedulas_bi = "CEDINGBEDE" + formulario.date_start + ' al ' + formulario.date_end + '.txt' 
      filename_cedulas_be = "CEDGASGBEDE" + formulario.date_start + ' al ' + formulario.date_end + '.txt' 
      ...
      writer_bal_xls = XLSWriter.XLSWriter()
      cabecera_bal_xls = ['Mes','Cuenta Codigo','Debe Inicial','Haber Inicial','Flujo Debe','Flujo Haber','Debe Suma','Haber Suma','Debe Final','Haber Final']
      writer_bal_xls.append(cabecera_bal_xls)
      di=hi=df=hf=ds=hs=df1=hf1=0 
      for line in dic_ord:
          if line['code']!=0 and line['esigef']:
              if len(line['code']) < 7:
                  ...
                  code = line['code']
              if line['debe_inicial'] or line['haber_inicial'] or line['debe_flujo'] or line['haber_flujo'] or line['debe_suma'] or line['haber_suma'] or line['debe_final'] or line['haber_final']:
                  ...
                  aux_name = unicodedata.normalize('NFKD',line['desc']).encode('ascii','ignore')
                  datas_be.append(['68070000',mes,code[0:7],aux_name,'%.2f'%line['debe_inicial'],'%.2f'%line['haber_inicial'],'%.2f'%line['debe_flujo'],'%.2f'%line['haber_flujo'],'%.2f'%line['debe_suma'],'%.2f'%line['haber_suma'],'%.2f'%line['debe_final'],'%.2f'%line['haber_final']])
                  #al excel
                  di+=line['debe_inicial']
                  hi+=line['haber_inicial']
                  df+=line['debe_flujo']
                  hf+=line['haber_flujo']
                  ds+=line['debe_suma']
                  hs+=line['haber_suma']
                  df1+=line['debe_final']
                  hf1+=line['haber_final']
                  detalle_cuenta_xls = [mes,code[0:7],line['debe_inicial'],line['haber_inicial'],line['debe_flujo'],line['haber_flujo'],line['debe_suma'],line['haber_suma'],line['debe_final'],line['haber_final']]
                  writer_bal_xls.append(detalle_cuenta_xls)
      writer_bal_xls.append(['','',di,hi,df,hf,ds,hs,df1,hf1])
      writer_bal_xls.save("balanceEsigef.xls")
      writer_bal_xls.save("balanceEsigef.xls")
      out_bal_xls = open("balanceEsigef.xls","rb").read().encode("base64")
      datas_be_buf = StringIO.StringIO() 
      writer_be = csv.writer(datas_be_buf, delimiter='\t')
      writer_be.writerows(datas_be)
      out_balance_be = base64.encodestring(datas_be_buf.getvalue())
      datas_be_buf.close()
      budget_buf = StringIO.StringIO() 
      writer = csv.writer(budget_buf, delimiter='|')
      ...
      budget_buf.close()
      name_balance = 'BALANCE COMP-%s.txt' % (mes_aux)
      name_balance_be = 'BALANCEBEDE %s.txt' % (mes_aux)
      ...
      filename_trans = "ESIGEF TRANSFERENCIAS-%s.txt"%(mes_aux)
      codes_trans = ['11318','11328','21358']
      dict_trans = {'11318': '9999999999999','11328': '9999999999999','21358': ruc_company}
      accounts = self.pool.get('account.account').search(cr, uid, [('code','in',codes_trans)])
      ctx_trans = context.copy()
      ctx_trans.update({'fiscalyear_id': [formulario.fiscalyear_id.id], 'date_from': formulario.date_start, 'date_end': formulario.date_end, 'account_ids': accounts})
      ...
      for code in codes_trans:
          ...
          if debe_t!=0 or haber_t!=0:
              if code[0:5] in ('11318','11328'):
                  data_trans.append([mes,code[:3],code[3:5],'00',ruc_company,dict_trans[code],debe_t,haber_t,0])
              else:
                  data_trans.append([mes,code[:3],code[3:5],'00','9999999999999',ruc_company,debe_t,haber_t,0])
      writer_trans.writerows(data_trans)
      out_trans = base64.encodestring(trans_buf.getvalue())
      trans_buf.close()
      self.write(cr, uid, ids, {
          'data_balance_xls':out_bal_xls,
          'filename_balance_xls':'balanceEsigef.xls',
          ...
          'data_cedulas_bi': out_cedulas_bi,
          'filename_cedulas_bi': filename_cedulas_bi,
          'data_cedulas_be': out_cedulas_be,
          'filename_cedulas_be': filename_cedulas_be,
          'data_balance_be': out_balance_be,
          'filename_balance_be': name_balance_be,
          'mensajes': mensajes})              
      return True
...
#            budget_ids = budget_item_obj.search(cr, uid, [('code','=like',budget_txt+"%")])
    ...
    def crear_padre(self, data, data_suma, res,aux):
    ...
    #                    'program':data['post'].parent_id.code,
                ...
                self.crear_padre(res[data['post'].parent_id.code], data_suma,res,aux)
            else:
                if res.get(data['post'].parent_id.code,False):
                    res[data['post'].parent_id.code]['planned_amount'] += data_suma['planned_amount']
                    res[data['post'].parent_id.code]['devengado_amount'] += data_suma['devengado_amount']
        ...
        if codificacion_ids:
            ...
            if code_budget=='Si':
                ...
                for line in c_b_lines_obj.browse(cr,uid,ids_lines, context=contexto):
                    band_aux = True
                    if (line.planned_amount>0 or line.codif_amount>0 or line.commited_amount>0 or line.paid_amount>0 or line.devengado_amount>0) or band_aux:
...
#                            }
                            ...
                            #self.crear_padre(res_line[line.code[0:6]], res_line[line.code[0:6]],res_line,aux)
                        else:   
                            #if res_line[line.budget_post_id.code[0:6]]['orientacion']=='99999999':
                            #    orientacion_ids = orientacion_obj.search(cr, uid, [('budget_id','=',line.id)])
                            #    if orientacion_ids:
                            #        orientacion = orientacion_obj.browse(cr, uid, orientacion_ids[0])
                            #        orientacion_aux = orientacion.estrategy_id.sequence
                            #        res_line[line.budget_post_id.code[0:6]]['orientacion']=orientacion_aux[0:6] + '00'
            ...
            for line in c_b_lines_obj.browse(cr,uid,ids_lines, context=contexto):
                if res_line.has_key(line.code)==False:
                    ...
                    self.crear_padre(res_line[line.code], res_line[line.code],res_line,aux)
                else:              
...
class mayorCuentaLine(osv.TransientModel):
    ...
    _columns = dict(
        ...
        tipo_invoice = fields.char('Tipo',size=32),#fields.selection([('Bien','Bien'),('Servicio','Servicio')],'Tipo'),
...
class mayorLine(osv.TransientModel):
    _name = 'mayor.line'
    _columns =dict(
        t_i = fields.float('Total I'),
        t_g = fields.float('Total G'),
        inicial_debe = fields.float('Apertura Debe'),
        inicial_haber = fields.float('Apertura Haber'),
    ...
    def exportaExcel(self, cr, uid, ids, name, context={}):
        for this in self.browse(cr, uid, ids):
            ...
            for line_account in this.line_ids:
                ...
                #cabecera_all2 = ['','Fecha','RUC Beneficiario','Beneficiario','Descripcion','Tipo','Comprobante','Debe','Haber','Saldo']
...
#                    detalle_cuenta = ['',line.date,line.partner_id.ced_ruc,line.partner_id.name,line.name,line.tipo_invoice,line.doc,line.debit,line.credit,line.saldo]
    ...
    def loadMayor(self, cr, uid, ids, context=None):
        if not context:
            context = {}
        mayor_obj = self.pool.get('mayor.cuenta')
        move_line_obj = self.pool.get('account.move.line')
        account_obj = self.pool.get('account.account')
        ...
        for this in self.browse(cr, uid, ids):
            ###busca asientos posted que tengan lineas no valid y les pone valid
            move_line_ids_valid = move_line_obj.search(cr, uid, [('state','!=','valid'),('move_id.state','=','posted')])
            if move_line_ids_valid:
                move_line_obj.write(cr,uid, move_line_ids_valid,{'state':'valid'})
            ###
            antes_ids = line_obj.search(cr, uid, [('m_id','=',this.id)])
            line_obj.unlink(cr, uid, antes_ids)
            if this.all_hijas:
                account_ids = account_obj.search(cr, uid, [('code','=like',this.name_to.code+"%"),('type','!=','view')])
            else:
                account_ids = account_obj.search(cr, uid, [('code','>=',this.name.code),('code','<=',this.name_to.code),('type','!=','view')])
            aux_debe_m = aux_haber_m = aux_total_m = 0
            if account_ids and this.all_hijas and this.agua:
                seq_aux = 0
                ...
                ctx['state'] = 'posted'
                account = account_obj.read(cr, uid, this.name.id, ['id','type','code','name','debit','credit','balance','parent_id','level'], ctx)
                saldo_inicial = abs(account['balance'])
                #if account['code'][0:3] in ('112','911'):
                    #saldo_inicial = account['balance']
                lineas_inicial_ids = move_line_obj.search(cr, uid, [('account_id','=',this.name.id),('date','>=',this.date_start),('is_start','=',True),
                ...
                if lineas_inicial_ids:
                    linea_inicial = move_line_obj.browse(cr, uid, lineas_inicial_ids[0])
                    band_start = True
                move_line_ids = move_line_obj.search(cr, uid, [('account_id','in',account_ids),('date','>=',this.date_start),('is_start','=',False),
                                                               ('date','<=',this.date_end),('move_id.state','=','posted'),('state','=','valid')]
                ...
                if move_line_ids:
                    ...
                    #saldo_final = saldo_inicial + debe_aux_f - haber_aux_f
                    ctx2['date_to'] = this.date_end
                    ctx2['fiscalyear'] = this.fiscalyear_id.id
                    ctx2['date_from'] = this.date_start
                    ctx2['state'] = 'posted'
                    account2 = account_obj.read(cr, uid, this.name.id, ['id','type','code','name','debit','credit','balance','parent_id','level'], ctx2)
                    saldo_final = abs(account2['balance'])
                    saldo_inicial = abs(account['balance'])
                    #if account['code'][0:3] in ('112','911'):
                        #saldo_inicial = account['balance']
                    line_id = line_obj.create(cr, uid, {
                        'm_id':this.id,
                        'account_id':this.name.id,
                        'saldo_anterior':saldo_inicial,
                        'debe':account2['debit'],#debe_aux_f,
                        'haber':account2['credit'],#haber_aux_f,
                        'saldo_final':saldo_final,
                    })
                    #sumar al wizard
                    aux_debe_m += abs(debe_aux_f)
                    aux_haber_m += abs(haber_aux_f)
                    #
                    saldo_aux = 0
                    ...
                    for move_line_id in move_line_ids:
                        ...
                        if move_line.debit!=0 or move_line.credit!=0:
                            ...
                            #considerar si es de activo o pasivo
                            if move_line.debit!=0:
                                if move_line.account_id.code[0:3] in ('112','131','141','151'):
                                    next_saldo += move_line.debit
                                elif move_line.account_id.code[0:3]=='111':
                                    next_saldo += move_line.debit
                                elif move_line.account_id.code[0:3]=='113':
                                    next_saldo += move_line.debit
                                elif move_line.account_id.code[0:7] in ('2120304','2120101'):
                                    next_saldo -= move_line.debit
                                elif move_line.account_id.code[0:3]=='124':
                                    next_saldo += move_line.debit
                                elif move_line.account_id.code[0:3]=='911':
                                    next_saldo += move_line.debit
                                else:
                                    next_saldo -= move_line.debit
                            elif move_line.credit!=0:
                                if move_line.account_id.code[0:3]=='111':
                                    next_saldo -= move_line.credit
                                elif move_line.account_id.code[0:3]=='113':
                                    next_saldo -= move_line.credit
                                elif move_line.account_id.code[0:3] in ('112','131','141','151'):
                                    next_saldo -= move_line.credit
                                elif move_line.account_id.code[0:3]=='124':
                                    next_saldo -= move_line.credit
                                elif move_line.account_id.code[0:7] in ('2120304','2120101'):
                            ...
                            if move_line.budget_id_cert:
                                if move_line.budget_id_cert.tipo_invoice:
                                    if move_line.budget_id_cert.tipo_invoice in ('Bien','Iva'):
                                        aux_tipo = 'Bien'
                                    else:
                                        aux_tipo = 'Servicio'
                ...
                #            'saldo_anterior':0,
                #            'debe':linea_inicial.debit,
                #            'haber':linea_inicial.credit,
                #            'saldo_final':linea_inicial.debit-linea_inicial.credit,
                #            'inicial_debe':linea_inicial.debit,
                #            'inicial_haber':linea_inicial.credit,
                #        })
                    aux_total_m = abs(aux_debe_m - aux_haber_m)
                mayor_obj.write(cr, uid, this.id,{
                    'debe_m':aux_debe_m,
                    'haber_m':aux_haber_m,
                    'saldo_m':aux_total_m,
                })    
            elif account_ids and this.all_hijas:
                for account_id in account_ids:
                    ...
                    #if account['code'][0:3] in ('112','911'):
                        #saldo_inicial = account['balance']
                    lineas_inicial_ids = move_line_obj.search(cr, uid, [('account_id','=',account_id),('date','>=',this.date_start),('is_start','=',True),
                                                                        ('date','<=',this.date_end),('move_id.state','=','posted'),
                                                                        ('state','=','valid')],order='date asc')
                    band_start = False
                    debe_aux_f = haber_aux_f = 0
                    ...
                    if move_line_ids:
                        ...
                        ctx2['date_from'] = this.date_start
                        ctx2['state'] = 'posted'
                        account2 = account_obj.read(cr, uid, account_id, ['id','type','code','name','debit','credit','balance','parent_id','level'], ctx2)
                        saldo_final = abs(account2['balance'])
                        saldo_inicial = abs(account['balance'])
                        #if account['code'][0:3] in ('112','911'):
                            #saldo_inicial = account['balance']
                            ...
                            if move_line.debit!=0 or move_line.credit!=0:
                                ...
                                if move_line.debit!=0:
                                    if move_line.account_id.code[0:3] in ('112','131','141','151'):
                                        ...
                                        next_saldo += move_line.debit
                                    elif move_line.account_id.code[0:1]=='1':
                                        next_saldo += move_line.debit
                                    else:
                                        next_saldo -= move_line.debit
                                elif move_line.credit!=0:
                    ...
                    if this.partner_id:
                        if this.tipo=='Todos':
                            move_line_ids = move_line_obj.search(cr, uid, [('account_id','=',account_id),('date','>=',this.date_start),('is_start','=',False),
                                                                           ('date','<=',this.date_end),('move_id.state','=','posted'),
                                                                           #('date','!=',this.fiscalyear_id.date_start),
                                                                           ('state','=','valid'),('partner_id','=',this.partner_id.id)],order='date asc,move_id asc')
                        elif this.tipo=='Ingresos':
                            move_line_ids = move_line_obj.search(cr, uid, [('account_id','=',account_id),('date','>=',this.date_start),('is_start','=',False),
                                                                           ('date','<=',this.date_end),('move_id.state','=','posted'),
                                                                           #('date','!=',this.fiscalyear_id.date_start),
                                                                           ('debit','>',0),
                        ...
                        if this.tipo=='Todos':
                            move_line_ids = move_line_obj.search(cr, uid, [('account_id','=',account_id),('date','>=',this.date_start),('is_start','=',False),
                                                                           ('date','<=',this.date_end),('move_id.state','=','posted'),
                                                                           #('date','!=',this.fiscalyear_id.date_start),
                                                                           ('state','=','valid')],order='date asc,move_id asc')
                        elif this.tipo=='Ingresos':
                            move_line_ids = move_line_obj.search(cr, uid, [('account_id','=',account_id),('date','>=',this.date_start),('is_start','=',False),
                                                                           ('date','<=',this.date_end),('move_id.state','=','posted'),
                                                                           #('date','!=',this.fiscalyear_id.date_start),
                    ...
                    if move_line_ids:
                        ...
                        for move_line_id in move_line_ids:
                            ...
                            if move_line.debit!=0 or move_line.credit!=0:
                                ...
                                #considerar si es de activo o pasivo
                                if move_line.debit!=0:
                                    if move_line.account_id.code[0:3] in ('112'):
                                        #if next_saldo<0:
                                        #    next_saldo = abs(next_saldo)
                                        next_saldo += move_line.debit
                                    elif move_line.account_id.code[0:3] in ('131','141','151'):
                                        next_saldo += move_line.debit
                                    elif move_line.account_id.code[0:3]=='111':
                                        next_saldo += move_line.debit
                                    elif move_line.account_id.code[0:3]=='113':
    ...
    def print_mayor_cta(self, cr, uid, ids, context=None):
        ...
        for this in self.browse(cr, uid, ids):
            if this.all_hijas and this.agua:
                return {
                    'type': 'ir.actions.report.xml',
...
#                }
    ...
    def print_mayor_cta_saldo(self, cr, uid, ids, context=None):
        if not context:
            context = {}
        report = self.browse(cr, uid, ids, context)[0]
        datas = {'ids': [report.id], 'model': 'mayor.cuenta'}
        return {
            'type': 'ir.actions.report.xml',
            'report_name': 'mayor.cuenta.saldo',
            'model': 'mayor.cuenta',
            'datas': datas,
            'nodestroy': True,                        
        }
    ...
    _columns = dict(
        agua = fields.boolean('Mayor Consolidado'),
        all_hijas = fields.boolean('Detallado Ultimo Nivel'),
        inicial_debe = fields.float('Inicial Debe'),
        inicial_haber = fields.float('Inicial Haber'),
        debe_m = fields.float('Debe'),
        haber_m = fields.float('Haber'),
        saldo_m = fields.float('Saldo'),
        tipo = fields.selection([('Ingresos','Ingresos'),('Egresos','Egresos'),('Todos','Todos')],'Opcion'),
