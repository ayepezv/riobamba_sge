...
class reqLinePac(osv.Model):
    _inherit = 'purchase.requisition.line'
    _columns = dict(
        pac_line_id = fields.many2one('purchase.pac.line','Producto'),
    )
...
#                    qty = abs(qty_stock - qty)
    ...
    def tender_in_progress(self, cr, uid, ids, context=None):
        ...
        for this in self.browse(cr, uid, ids):
            user_actual = user_obj.browse(cr, uid, uid)
            if not user_actual.context_department_id.id == this.pac_id.department_id.id and not user_actual.id==1:
                raise osv.except_osv(('Error Usuario'), ('Usted no pertenece al departamento del PAC seleccionado'))
