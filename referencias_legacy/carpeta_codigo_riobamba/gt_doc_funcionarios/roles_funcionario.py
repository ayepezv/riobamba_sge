...
class roles_empleado_line(osv.TransientModel):
    ...
    _columns = dict(
        ...
        name = fields.char('Rol Descripcion',size=256),
...
class roles_empleado(osv.TransientModel):
   ...
   def mostrar_roles(self, cr, uid, ids, context=None):
       ...
       for this in self.browse(cr, uid, ids):
           ...
           if this.periodo_id:
               roles_ids = payslip_obj.search(cr, uid, [('employee_id','=',this.empleado_id.id),('period_id','=',this.periodo_id.id),('state','=','Pagado')])
           else: 
               roles_ids = payslip_obj.search(cr, uid, [('employee_id','=',this.empleado_id.id),('state','=','Pagado')])
           ...
           if roles_ids:
               for line_id in roles_ids:
                   ...
                   line_rol_obj.create(cr, uid, {
                       ...
                       'total':roles.net,
                       'rol_id':line_id,
                       'name':roles.payslip_run_id.name,
                   })
           else:
