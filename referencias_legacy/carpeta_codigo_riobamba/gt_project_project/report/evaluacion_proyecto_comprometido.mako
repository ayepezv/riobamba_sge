...
<table WIDTH="100%" border="1" cellpadding="2" cellspacing="0"  bordercolor="#000000" style="border-collapse:collapse;font-size:18px">
  <thead style="display: table-header-group" BGCOLOR="#D8D8D8">   
    ...
    <td align="center" style="font-size:18px" WIDTH="8%">Comp. Mes</td>
    <td align="center" style="font-size:18px" WIDTH="4%">%</td>
    <td align="center" style="font-size:18px" WIDTH="8%">Comp. Corte</td>
    <td align="center" style="font-size:18px" WIDTH="4%">%</td>
  ...
  <tr style="border: 1px solid black; page-break-inside: avoid;font-size:14px">
    ...
    %if values['commited_amount_mes']<=0:
    <td WIDTH="8%" align="right">${'{:,.2f}'.format(values['commited_amount_mes'])}</td>
    <td WIDTH="4%" align="right">${'{:,.2f}'.format(values['percent_rec_mes'])}</td>	    
    %else:
    <td WIDTH="8%" align="right">${'{:,.2f}'.format(values['commited_amount_mes'])}</td>
    <td WIDTH="4%" align="right">${'{:,.2f}'.format(values['percent_rec_mes'])}</td>	    
    %endif
    <td WIDTH="8%" align="right">${'{:,.2f}'.format(values['commited_amount'])}</td>
    <td WIDTH="4%" align="right">${'{:,.2f}'.format(values['percent_rec'])}</td>	    
    <td WIDTH="8%" align="right">${'{:,.2f}'.format(abs(values['to_rec']))}</td>
    <td WIDTH="4%" align="right">${'{:,.2f}'.format(abs(values['percent_sal']))}</td>
  ...
  <tr style="border: 1px solid black; page-break-inside: avoid;font-size:14px">
    ...
    %if values['commited_amount_mes']<=0:
    <td WIDTH="8%" align="right"><b>${'{:,.2f}'.format(values['commited_amount_mes'])}</b></td>
    <td WIDTH="4%" align="right"><b>${'{:,.2f}'.format(values['percent_rec'])}</b></td>	    
    %else:
    <td WIDTH="8%" align="right"><b>${'{:,.2f}'.format(values['commited_amount_mes'])}</b></td>
    <td WIDTH="4%" align="right"><b>${'{:,.2f}'.format(values['percent_rec_mes'])}</b></td>	    
    %endif
    <td WIDTH="8%" align="right"><b>${'{:,.2f}'.format(values['commited_amount'])}</b></td>
    <td WIDTH="4%" align="right"><b>${'{:,.2f}'.format(values['percent_rec'])}</b></td>	    
    <td WIDTH="8%" align="right"><b>${'{:,.2f}'.format(abs(values['to_rec']))}</b></td>
    <td WIDTH="4%" align="right"><b>${'{:,.2f}'.format(abs(values['percent_sal']))}</b></td>
  ...
  <tfoot style="display: table-row-group">
    <%
       if res['total']['codif_amount']>0:
           ...
           por_pagado = res['total']['commited_amount']
           por_pagado_mes = res['total']['commited_amount_mes']
       saldo_acumulado = res['total']['codif_amount'] - res['total']['commited_amount']
       por_saldo = 100 - por_pagado
       %>
    <td WIDTH="10%" style="font-weight: bold;font-size:14px" align="right"></td>  
    <td WIDTH="20%" style="font-weight: bold;font-size:14px" align="right">TOTALES</td>        
    <td WIDTH="9%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(res['total']['planned_amount'])}</td>
    %if abs(res['total']['reform_amount'])>0.01:
    <td WIDTH="9%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(res['total']['reform_amount'])}</td>
    %else:
    <td WIDTH="9%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(0)}</td>
    %endif
    <td WIDTH="9%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(res['total']['codif_amount'])}</td>    
    %if res['total']['commited_amount_mes']<=0:
    <td WIDTH="8%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(res['total']['commited_amount_mes'])}</td>    
    <td WIDTH="4%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(por_pagado_mes)}</td>    
    %else:
    <td WIDTH="8%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(res['total']['commited_amount_mes'])}</td>    
    <td WIDTH="4%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(por_pagado_mes)}</td>    
    %endif
    <td WIDTH="8%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(res['total']['commited_amount'])}</td>    
    <td WIDTH="4%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(por_pagado)}</td>    
    <td WIDTH="8%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(saldo_acumulado)}</td>    
    <td WIDTH="4%" style="font-weight: bold;font-size:14px" align="right">${'{:,.2f}'.format(por_saldo)}</td>    
    <td WIDTH="20%" style="font-weight: bold;font-size:14px" align="right"></td>    
  <tfoot>
</table>
%endfor
</body>
