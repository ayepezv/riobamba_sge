...
class evaluacion_proyecto(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context=None):
        super(evaluacion_proyecto, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'time': time,
            '_vars': self._vars,
            '_get_totales':self._get_totales,
            '_get_totales_egreso':self._get_totales_egreso,
           '_get_anio':self._get_anio,
        })

    def _get_anio(self, code,name):
        post_obj = self.pool.get('budget.post')
        aux = ''
        if len(code)>8:
            post_ids = post_obj.search(self.cr, self.uid, [('code','=like',code[:len(code)-4]+"%")])
            if post_ids:
                post = post_obj.browse(self.cr, self.uid, post_ids[0])
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        ids_lines=c_b_lines_obj.search(self.cr,self.uid,[('poa_id','=',data['form']['poa_id'][0]),
                                                         ('type_budget','=','gasto'),('project_id','=',data['form']['project_id'][0])])
        if ids_lines:
            objects=self.pool.get('budget.poa').browse(self.cr,self.uid,data['form']['poa_id'][0])         
            return super(evaluacion_proyecto, self).set_context([objects], data, ids, report_type=report_type)
        else:
    ...
    def _vars(self,resumen):  
        ...
        res['end']=end.upper()
        res['completo'] = self.datas['form']['completo']
        res['project_name']=self.datas['form']['project_id'][1]
        return res 
        ...
        if data['post'].parent_id:
            ...
            if res.get(data['post'].parent_id.code,False):
                ...
                res[data['post'].parent_id.code] = {
                    ...
                    'cta_financia':data['cta_financia']
                }
    ...
    def _get_totales(self,resumen):
        ...
        project_id = self.datas['form']['project_id'][0]
        c_b_lines_obj = self.pool.get('budget.item')
        partida_id = self.datas['form']['budget_id']
        ids_lines = []
        if partida_id:
            ids_lines.append(partida_id[0])
        else:
            ids_lines=c_b_lines_obj.search(self.cr,self.uid,[('poa_id','=',resumen.id),('type_budget','=','gasto'),('project_id','=',project_id)])
        #cambiado mario
        aux_date_start = resumen.date_start
        #context = {'by_date':True,'date_start': date_from, 'date_end': date_to,'poa_id':resumen.id}  
        context = {'by_date':True,'date_start': aux_date_start, 'date_end': date_to,'poa_id':resumen.id}  
        ...
        for line in c_b_lines_obj.browse(self.cr,self.uid,ids_lines, context=context):
            ...
            sql_f = """select f.name from budget_certificate_line c,budget_financiamiento f where c.financia_id=f.id and c.date_commited>='%s' and c.date_commited<='%s' and c.budget_id=%s group by f.name order by name asc"""%(date_from,date_to,line.id)
            self.cr.execute(sql_f)
            aux_2 = ''
            for aux_name in self.cr.fetchall():
                aux_2 = aux_name[0] + ','
                aux_financia += aux_2
            if res_line.has_key(line.code)==False:
                ...
                aux_mes = line2.paid_amount
                if line2.paid_amount<=0:
                    aux_mes_ = line.paid_amount
                res_line[line.code]={
                ...
                if line.codif_amount>0:
                    ...
                    aux_percent_rec_mes = ((line2.paid_amount)*(100))/line.codif_amount
                    aux_percent_super = ((aux_superavit)*(100))/line.codif_amount
                res_line[line.code]['planned_amount']+=line.planned_amount#res_line[line.budget_post_id.id]['planned_amount']
...
report_sxw.report_sxw('report.evaluacion_proyecto','evaluacion.proyecto',
                      'gt_project_project/report/evaluacion_proyecto.mako',
                      parser=evaluacion_proyecto)


class evaluacion_proyecto_comprometido(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context=None):
        super(evaluacion_proyecto_comprometido, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'time': time,
            '_vars': self._vars,
            '_get_totales':self._get_totales,
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if ids_lines:
            objects=self.pool.get('budget.poa').browse(self.cr,self.uid,data['form']['poa_id'][0])         
            return super(evaluacion_proyecto_comprometido, self).set_context([objects], data, ids, report_type=report_type)
        else:
            raise osv.except_osv('Error!!', 'No existen partidas tipo gasto del presupuesto')
    ...
    def _get_totales(self,resumen):
        ...
        for line in c_b_lines_obj.browse(self.cr,self.uid,ids_lines, context=context):
            ...
            if res_line.has_key(line.code)==False:
                ...
                res_line[line.code]['to_rec']+=aux_to_pay
                #res_line[line.budget_post_id.id]['final']=False
        res_line['total']={
            'planned_amount': 0.00,
            'reform_amount': 0.00,
            'codif_amount': 0.00,
        ...
        values=res_line.itervalues()
        for line_totales in values:
            if line_totales.has_key('final') and line_totales['final']==True:
                res_line['total']['planned_amount']+=line_totales['planned_amount']            
                res_line['total']['reform_amount']+=line_totales['reform_amount']
                res_line['total']['codif_amount']+=line_totales['codif_amount']
                res_line['total']['paid_amount']+=line_totales['paid_amount']
...
#                if res_line['total']['commited_amount_mes']<=0:
#                    res_line['total']['commited_amount_mes']+=line_totales['commited_amount']
#                else:
#                    res_line['total']['commited_amount_mes']+=line_totales['commited_amount_mes']
                res_line['total']['to_rec']+=line_totales['to_rec']
                res_line['total']['percent_sal']+=line_totales['percent_sal']
...
report_sxw.report_sxw('report.evaluacion_proyecto_comprometido','evaluacion.proyecto.comprometido',
                      'gt_project_project/report/evaluacion_proyecto_comprometido.mako',
                      parser=evaluacion_proyecto_comprometido)
