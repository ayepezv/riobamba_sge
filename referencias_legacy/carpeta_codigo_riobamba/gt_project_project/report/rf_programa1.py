...
class rf_programa(report_sxw.rml_parse):
    ...
    def act_traspaso_aumento(self, data, data_suma, res):
        if data['post'].parent_id:
            data['padre'] = data['post'].parent_id
            if res.get(data['post'].parent_id.code,False):
                res[data['post'].parent_id.code]['traspaso_aumento'] += data_suma['traspaso_aumento']
            ...
            self.act_traspaso_aumento(res[data['post'].parent_id.code], data_suma,res)

    def act_suplemento(self, data, data_suma, res):
        if data['post'].parent_id:
            data['padre'] = data['post'].parent_id
            if res.get(data['post'].parent_id.code,False):
                res[data['post'].parent_id.code]['suplemento'] += data_suma['suplemento']
            ...
            self.act_suplemento(res[data['post'].parent_id.code], data_suma,res)

    def act_traspaso_disminucion(self, data, data_suma, res):
        if data['post'].parent_id:
            data['padre'] = data['post'].parent_id
            if res.get(data['post'].parent_id.code,False):
                res[data['post'].parent_id.code]['traspaso_aumento'] += data_suma['traspaso_aumento']
            ...
            self.act_traspaso_disminucion(res[data['post'].parent_id.code], data_suma,res)

    def act_disminucion(self, data, data_suma, res):
        if data['post'].parent_id:
            data['padre'] = data['post'].parent_id
            if res.get(data['post'].parent_id.code,False):
                res[data['post'].parent_id.code]['disminucion'] += data_suma['disminucion']
            ...
            self.act_disminucion(res[data['post'].parent_id.code], data_suma,res)
...
#                    line_reformas_ids = line_gasto_obj.search(self.cr, self.uid, [('budget_id','=',line.id)])
                    ...
                    if line_reformas_ids3:
                        line_r = line_gasto_obj.browse(self.cr, self.uid, line_reformas_ids3[0])
                        if line_r.mass_id:
                            if line_r.traspaso:
                    ...
                    line_reformas_ids2 = line_gasto_obj.search(self.cr, self.uid, [('mass_id2','=',reforma_id),('budget_id','=',line.id)])
                    line_reformas_ids3 = line_reformas_ids + line_reformas_ids2
                    if res_line_positions.has_key(line.code)==False:
                        res_line_positions[line.code] = 1
                    else:
                        res_line_positions[line.code] += 1

                    if line_reformas_ids3 and len(line_reformas_ids3)>res_line_positions[line.code]:
                        line_r = line_gasto_obj.browse(self.cr, self.uid, line_reformas_ids3[res_line_positions[line.code]])
                        if line_r.mass_id:
                            if line_r.traspaso:
                                res_line[line.code]['traspaso_disminucion'] += line_r.monto
                                self.act_traspaso_disminucion(res_line[line.code], res_line[line.code],res_line)
                            else:
                                res_line[line.code]['disminucion'] += line_r.monto
                                self.act_disminucion(res_line[line.code], res_line[line.code],res_line)
                        else:
                            if line_r.traspaso:
                                res_line[line.code]['traspaso_aumento'] += line_r.monto
                                self.act_traspaso_aumento(res_line[line.code], res_line[line.code],res_line)
                            else:
                                res_line[line.code]['suplemento'] += line_r.monto
                                self.act_suplemento(res_line[line.code], res_line[line.code],res_line)
                             res_line[line.code]['suplemento']+=line.monto
