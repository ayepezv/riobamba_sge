...
class cedulaCodificadoGasto(report_sxw.rml_parse):
    ...
    def _get_total_codif(self,resumen):
        ...
        aux_total_funcion = 0
        aux_code_programa = resumen.program_id.sequence[0]
        for line in c_b_lines_obj.browse(self.cr,self.uid,ids_lines, context=context):
            aux_codif = line.codif_amount
            aux_total_presupuesto += aux_codif
            if  line.code[0]==aux_code_programa:
                aux_total_funcion += aux_codif
        res_line['total_presupuesto'] = aux_total_presupuesto
        res_line['total_funcion'] = aux_total_funcion
        return res_line
