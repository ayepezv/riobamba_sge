...
class contract_sindicature(osv.Model):
    _name = 'contract.sindicature'
    _columns = dict(
        name = fields.many2one('res.users','Usuario Sindicatura',required=True),
        )
contract_sindicature()

class cpWarranty(osv.Model):
    _name = 'cp.warranty'
    _columns = dict(
        name = fields.char('Garantía',size=64,required=True),
        )
cpWarranty()

class PurchasePublicProcessInherit(osv.Model):
    _inherit = 'purchase.public.process'
    _order = 'date desc'
...
#                                           required=True),
        ...
        state = fields.many2one('purchase.process.state', string='Estado'),
