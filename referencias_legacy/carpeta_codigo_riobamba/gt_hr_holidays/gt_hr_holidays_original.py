...
class certificadoPermiso(osv.Model):
    ...
    def draft_phora_ejecutado_cert(self, cr, uid, ids, context=None):
        ...
        for this in self.browse(cr, uid, ids):
#            if uid!=user.context_department_id.coordinador_id.id:
...
#        usuario_autoriza = fields.related('jefe_id','user_id',type='many2one',relation='res.users',string="Usuario Autoriza",store=True),
    ...
    def solicitaPermisoHoli(self, cr, uid, ids, context=None):
        ...
        for this in self.browse(cr, uid, ids):
            ...
            if this.date > this.date_end and this.tipo == 'Dias':
                raise osv.except_osv('Error de usuario', 'La fecha inicio no puede ser mayor a la fecha fin')
            if this.hora_inicio > this.hora_fin and this.tipo == 'Horas':
                raise osv.except_osv('Error de usuario', 'La hora de inicio no puede ser mayor a la hora de fin')
...
#                raise osv.except_osv('Error de usuario', 'La fecha debe ser igual a la actual')
            ...
            if this.code=='/':
                obj_sequence = self.pool.get('ir.sequence')
                aux_code = obj_sequence.get(cr, uid, 'hr.holidays')
                self.write(cr, uid, ids[0],{'state':'Solicitado','code':aux_code,'usuario_autoriza':user_ids[0]})
            else:
                self.write(cr, uid, ids[0],{'state':'Solicitado','usuario_autoriza':user_ids[0]})
        return True
...
#        ]
    ...
    _defaults = dict(
        ...
        # date_permiso = datetime.datetime(time.localtime().tm_year, time.localtime().tm_mon, time.localtime().tm_mday).strftime('%Y-%m-%d'),
        date_permiso= fields.datetime.now,
        state = 'Borrador',
        tipo2 = 'Permiso',
        create_user_id = _get_uid,
    )
