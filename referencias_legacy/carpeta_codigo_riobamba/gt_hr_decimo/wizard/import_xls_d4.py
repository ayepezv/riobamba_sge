...
class import_sheet_d4(osv.osv_memory):

    _name='import.sheet.d4'
    ...
    def import_sheet_d4(self, cr, uid, ids, context=None):
        emp_obj=self.pool.get('hr.employee')
...
#        pdb.set_trace()
        ...
        parent=dec_obj.browse(cr, uid,context['active_id'])
        ids_unlink=[]
            ...
            for r in range(sh.nrows)[1:]:
                if sh.cell(r,0).value and sh.cell(r,1).value:
                    ...
                    if emp_id:
                        i += 1
                        aux_acumula = False
                        contract_ids = contract_obj.search(cr, uid, [('employee_id','=',emp_id),('activo','=',True)])
                        if contract_ids:
                        ...
                        for emp in emp_id:
                            ...
                            dec_line_obj.create(cr, uid, {
                                ...
                                'mar':sh.cell(r,2).value and sh.cell(r,2).value or 0.0,
                                'abr':sh.cell(r,3).value and sh.cell(r,3).value or 0.0,
                                'may':sh.cell(r,4).value and sh.cell(r,4).value or 0.0,
                                'jun':sh.cell(r,5).value and sh.cell(r,5).value or 0.0,
                                'jul':sh.cell(r,6).value and sh.cell(r,6).value or 0.0,
                                'ago':sh.cell(r,7).value and sh.cell(r,7).value or 0.0,
                                'sep':sh.cell(r,8).value and sh.cell(r,8).value or 0.0,
                                'oct':sh.cell(r,9).value and sh.cell(r,9).value or 0.0,
                                'nov':sh.cell(r,10).value and sh.cell(r,10).value or 0.0,
                                'dic':sh.cell(r,11).value and sh.cell(r,11).value or 0.0,
                                'ene':sh.cell(r,12).value and sh.cell(r,12).value or 0.0,
                                'feb':sh.cell(r,13).value and sh.cell(r,13).value or 0.0,
