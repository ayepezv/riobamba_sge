...
<table WIDTH="100%" border="1" cellpadding="2" cellspacing="0"  bordercolor="#000000" style="border-collapse:collapse;font-size:16px">
  <thead style="display: table-header-group" BGCOLOR="#D8D8D8">   
    <tr>
      ...
      <td style="font-size:14px" align="left" WIDTH="80%"><b>Del: ${vars['date_from']} Al: ${vars['date_to']}</b></td>    
      <td></td>
      <td></td>
      <td></td>
      <td></td>
    ...
    <tr>
      ...
      <td style="font-size:14px" align="left" WIDTH="80%"><b>${vars['mayor_name']} - ${vars['proyecto_code']}</b></td>
      %else:
      <td style="font-size:14px" align="left" WIDTH="80%"><b>${vars['proyecto_code']}</b></td>    
      %endif
      <td></td>
      <td></td>
      <td></td>
    ...
    <tr>
      ...
      <td style="font-size:14px" align="left" WIDTH="80%"><b>${vars['project_name']}</b></td>    
      <td></td>
      <td></td>
      <td></td>
      <td></td>
    ...
    <tr>
      ...
      <td align="center" style="font-size:16px" WIDTH="12%">REFORMAS</td>
      ...
      <td align="center" style="font-size:16px" WIDTH="7%">COMP. Corte</td>
  ...
  %if (values['nivel'] == 7 or values['nivel'] == 8) and (values['planned_amount']!=0 or values['reform_amount']!=0 or values['codif_amount']!=0 or values['commited_amount']!=0):
	  <tr style="border: 1px solid black; page-break-inside: avoid;">
	    ...
	    <td style="font-size:18px" WIDTH="7%" align="right">${'{:,.2f}'.format(values['planned_amount'])}</td>
	    <td style="font-size:18px" WIDTH="12%" align="right">${'{:,.2f}'.format(values['reform_amount'])}</td>
	    <td style="font-size:18px" WIDTH="7%" align="right">${'{:,.2f}'.format(values['codif_amount'])}</td>
	    <td WIDTH="8%" align="right">${'{:,.2f}'.format(values['commited_amount_mes'])}</td>
	    <td WIDTH="4%" align="right">${'{:,.2f}'.format(values['percent_rec_mes'])}</td>	    
	    <td style="font-size:18px" WIDTH="7%" align="right">${'{:,.2f}'.format(values['commited_amount'])}</td>
	    <td style="font-size:18px" WIDTH="7%" align="right">${'{:,.2f}'.format(values['percent_rec'])}</td>	    
	    <td style="font-size:18px" WIDTH="7%" align="right">${'{:,.2f}'.format(values['to_rec'])}</td>
	    <td style="font-size:18px" WIDTH="7%" align="right">${'{:,.2f}'.format(values['percent_sal'])}</td>
	  </tr>
 %endif
  %endif
  %endfor
...
<%
       ...
       %>
<td WIDTH="15%" style="font-weight: bold;font-size:18px" align="right"></td>  
...
<td WIDTH="12%" style="font-weight: bold;font-size:18px" align="right">${'{:,.2f}'.format(res['total']['reform_amount'])}</td>
<td WIDTH="7%" style="font-weight: bold;font-size:18px" align="right">${'{:,.2f}'.format(res['total']['codif_amount'])}</td>  
<td WIDTH="8%" align="right">${'{:,.2f}'.format(res['total']['commited_amount_mes'])}</td>
<td WIDTH="4%" align="right">${'{:,.2f}'.format(por_pagado_mes)}</td>	    
<td WIDTH="7%" style="font-weight: bold;font-size:18px" align="right">${'{:,.2f}'.format(res['total']['commited_amount'])}</td>    
