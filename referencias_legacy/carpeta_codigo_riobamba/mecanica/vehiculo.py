...
class quitar_caja(osv.TransientModel):
    _name = 'quitar.caja'
    _columns = {
        'caja_id_quitar': fields.many2one('caja.chica.solicitud', 'Quitar Caja Chica', required=True),
    }
    def quitar_caja_chica(self, cr, uid, ids, context=None):
        vehiculo_obj = self.pool.get('vehiculo')# objeto vehiculo paso a la variabble vehiculo_obj
        mantenimiento_line_obj = self.pool.get('vehiculo.mantenimiento')
        lubricante_line_obj = self.pool.get('vehiculo.lubricante')
        ...
        for this in self.browse(cr, uid, ids):# cargo todo el formulario de este mismo objeto osea los campos en este form 
            move_lines = context and context.get('active_id', False) or False#exactamente que hace eso 
            requerimientom = caja_obj.browse(cr, uid, move_lines)#  que hace     
            caja_obj.write(cr, uid, this.caja_id_quitar.id,{'flag':False,})#PONE EL REQUERIMIENTO EL CAMPO FLAG EN FALSO
            lubri_ids_antes = lubricante_line_obj.search(cr, uid, [('requerimiento_quitar_lubri','=',this.caja_id_quitar.name)])
            manten_ids_antes = mantenimiento_line_obj.search(cr, uid, [('requerimiento_cajachica','=',this.caja_id_quitar.name)])
            lubricante_line_obj.unlink(cr, uid, lubri_ids_antes)
            mantenimiento_line_obj.unlink(cr, uid, manten_ids_antes)
        return {'type':'ir.actions.act_window_close' }
quitar_caja()
...
class import_caja_mantenimiento(osv.TransientModel):
    ...
    def import_caja_mantenimiento_vehiculo(self, cr, uid, ids, context=None):
        ...
        for this in self.browse(cr, uid, ids):# cargo todo el formulario de este mismo objeto
            ...
            caja_obj.write(cr, uid, this.caja_id_mant.id,{'flag':'True',})
            for line in this.caja_id_mant.line_ids:# 
                nombre_pro = line.name
                if nombre_pro.startswith("ACEITE") or nombre_pro.startswith("aceite") or nombre_pro.startswith("Aceite"):
                    move2_id = lubricante_line_obj.create(cr, uid, { 
                        'descripcion_lubricante':line.name,#paso el nombre de la descripcion del detalle de la caja chica
		                'cantidad':line.qty,
                        'vehiculol_id':vehiculom.id,#que hace
                        'fecha':this.caja_id_mant.date,
                        'requerimiento_quitar_lubri':this.caja_id_mant.name,
                        'unidad_id':line.unidad.id,
                })
                else: #si el item de requerimiento no empieza con aceite ingresa a mantenimiento
                    move_id = mantenimiento_line_obj.create(cr, uid, { 
                        'name':line.name,#paso nombre del product q pertenece ala tabla stock.requisition.line y esta a producto
		                'cantidad':line.qty,
                        'vehiculom_id':vehiculom.id,
...
class vehiculo_lubricante(osv.Model):
	...
	fecha=fields.date('Fecha'),
        ...
        requerimiento_quitar_lubri = fields.char('Requerimiento',size=10),
...
class vehiculo(osv.Model):
    ...
    def _check_vehiculo_unico(self, cr, uid, ids):
        ...
        for vehi in self.browse(cr, uid, ids):
            vehiculo_ids = vehiculo_obj.search(cr, uid, [('activo_vehiculo_id','=',vehi.activo_vehiculo_id.id),('id','!=',vehi.id)]) 
            if vehiculo_ids:
                return False
        return True
