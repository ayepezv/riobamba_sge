        ...
        if account_parent_ids and len(lista)==0:
            ...
            if abs(account_parent[0]['balance'])>0:
                ...
                if account_hijas_ids:
                    for account_hija in account_obj.read(self.cr, self.uid, account_hijas_ids,['code','code_aux','name','debit','credit', 'balance', 'parent_id','level'], ctx):
                        if abs(account_hija['balance'])>0:
                            account_hija_ant = account_obj.read(self.cr, self.uid, account_hijas_ids,['code','code_aux','name','debit','credit', 'balance', 'parent_id','level'], ctx_ant)
                            aux_hija = []
            ...
            for account_id in lista:
                ...
                if account_hija_ids:
                    account_hija = account_obj.read(self.cr, self.uid, account_hija_ids[0],['code','code_aux','name','debit','credit', 'balance', 'parent_id','level'], ctx)
                    if abs(account_hija['balance'])>0:
                        account_hija_ant = account_obj.read(self.cr, self.uid, account_hija_ids[0],['code','code_aux','name','debit','credit', 'balance', 'parent_id','level'], ctx_ant)    
                        aux_hija = []
                        aux_hija.append(account_hija['code_aux'])
                        aux_hija.append(account_hija['name'])
                        aux_hija.append(abs(account_hija['balance']))
...
class EstadoSituacion(report_sxw.rml_parse):
    ...
    def lineas_individual(self, resumen, tipo):
        ...
        codes = ['111','112','113','121','122','123','124','132','134','135','141','14199','142','14299','144','14499','145','14599','151','15198',
                 '152','15298','125','12599','126','12699','131','133','212','213','221','222','223','224','225','611','612','61801','619','618','911',
                 '921','62','63']
        account_ids = account_obj.search(cr, uid, [('code','in', codes)])
        ctx.update({'fiscalyear': fiscalyear_id})
        ctx.update({'state': 'posted'})
...
#        inversiones = accounts_by_id['151']['balance']+accounts_by_id['15198']['balance']+accounts_by_id['152']['balance']+accounts_by_id['15298']['balance']
        ...
        potros = accounts_by_id['225']['balance']
...
#                date_final_reporte = fiscal_anterior.date_stop
        ...
        usos_corrientes = ['21351','21353','21355','21356','21357','21358']
    ...
    def get_saldo_flujo(self, code,desde,hasta,tipo):
        ...
        if account_parent_ids:
            ...
            if abs(account_parent[0][tipo])>0:
                ...
                if account_hijas_ids:
                    for account_hija in account_obj.read(self.cr, self.uid, account_hijas_ids,['code','code_aux','name','debit','credit', 'balance', 'parent_id','level'], ctx):
                        account_hijaa = account_obj.read(self.cr, self.uid, account_hijas_ids,['code','code_aux','name','debit','credit', 'balance', 'parent_id','level'], ctx2)
