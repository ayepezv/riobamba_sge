...
class fondoProyecto(osv.TransientModel):
    ...
    def exporta_excel_fppy(self, cr, uid, ids, context=None):
        ...
        for this in self.browse(cr, uid, ids):
            ...
            writer.append(['PARTIDA','INICIAL','REFORMA','CODIFICADO','COMPROMETIDO','SALDO'])
            total_ini = total_r = total_cod = total_c = total_s = aux = 0
            for line in this.line_ids:
                total_ini+=line.inicial
                total_r += line.reforma
                total_cod += line.codificado
                total_c+=line.comprometido
                total_s += line.saldo
                aux += 1
                aux_budget = line.budget_id.code + ' - ' + line.budget_id.name
                writer.append([aux_budget,line.inicial,line.reforma,line.codificado,line.comprometido,line.saldo])
            writer.append(['TOTAL',total_ini,total_r,total_cod,total_c,total_s])
...
#            hoy = date 
            ...
            if line.mass_id:
                res[line.id] = line.inicial - line.monto # budget.codif_amount - line.monto
            else:
                res[line.id] = line.inicial + line.monto # budget.codif_amount - line.monto
        return res
    ...
    def _amount_reforma(self, cr, uid, ids, prop, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            budget = item_obj.browse(cr, uid, line.budget_id.id, context=context)
            if line.mass_id:
                aux_codificado = budget.codif_amount - line.monto
            else:
                aux_codificado = budget.codif_amount + line.monto
            res[line.id]['reformas'] = line.monto#budget.reform_amount
            res[line.id]['inicial']  = budget.codif_amount
            res[line.id]['codificado']  = aux_codificado
        return res
...
#        inicial = fields.related('budget_id', 'codif_amount', type="float",
#                                 string="Presupuesto", store=True,readonly=True),
        inicial = fields.function(_amount_reforma, string='Inicial', multi="rf",store=True),
...
class massReform(osv.Model):
    ...
    def _amount_planned(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for this in self.browse(cr, uid, ids):
            ...
            item_ids = item_obj.search(cr, uid, [('type_budget','=','gasto'),('poa_id','=',poa_ids[0])])
...
class massReformLineIngreso(osv.Model):
    ...
    def _amount_final(self, cr, uid, ids, prop, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            budget = item_obj.browse(cr, uid, line.budget_id.id, context=context)
            if line.mass_id:
                res[line.id] = budget.codif_amount - line.monto #line.budget_id.planned_amount - line.monto
            else:
                res[line.id] = budget.codif_amount + line.monto # line.budget_id.planned_amount + line.monto
        return res
...
#                'final':0.0,
            ...
            res[line.id]['inicial']  = budget.codif_amount
            res[line.id]['codificado']  = aux_codificado
#            res[line.id]['final']  = budget.codif_amount
        return res
...
#                                 string="Asignacion Inicial", store=True,readonly=True),
        ...
        reformas = fields.function(_amount_reforma, string='Reformas', multi="rf",store=True),
        codificado = fields.function(_amount_reforma, string='Codificado', multi="rf",store=True),
#        final = fields.function(_amount_reforma,type="float",multi="rf", store=True,string='Presupuesto Final',readonly=True),
        final = fields.function(_amount_final,type="float", store=True,string='Presupuesto Final',readonly=True),
...
class budgetPoaFuncion(osv.Model):
    _name = 'budget.poa.funcion'
    _columns = dict(
        p_id = fields.many2one('budget.poa','Presupuesto'),
        funcion = fields.char('Funcion',size=1),
        total = fields.float('Total'),
    )
budgetPoaFuncion()

class poaPrograma(osv.Model):
    _name = 'poa.programa'
    _columns = dict(
        p_id = fields.many2one('budget.poa','Poa'),
        code = fields.char('Codigo',size=10),
        name = fields.char('Programa',size=128),
    )
poaPrograma()
...
class budgetPoa(osv.Model):
    ...
    def action_calcula_funcion(self, cr, uid, ids, context=None):
        ...
        for this in self.browse(cr, uid, ids):
            ...
            project_ids = project_obj.search(cr, uid, [('poa_id','=',this.id)])
            program_ids = []
            if project_ids:
                for project_id in project_ids:
                    project = project_obj.browse(cr, uid, project_id)
                    if not project.program_id.id in program_ids and project.program_id.sequence!='0':
                        program_ids.append(project.program_id.id)
#            program_ids = program_obj.search(cr, uid, [('sequence','!=','0'),('','',)])
            for program_id in program_ids:
                programa = program_obj.browse(cr, uid, program_id)
                sql_ = """select sum(planned_amount) from budget_item where poa_id=%s and program_id=%s"""%(this.id,programa.id)
                cr.execute(sql_)
                data = cr.fetchall()
                for line in data:
                    aux = line[0]
                if not programa.sequence[0] in dict_pr:
                    dict_pr[programa.sequence[0]]=aux
                else:
                    dict_pr[programa.sequence[0]]+=aux
            for linea_dict in dict_pr:
                line_obj.create(cr, uid, {
                    'funcion':linea_dict,
                    'total':dict_pr[linea_dict],
                    'p_id':this.id,
                })
        return True
    ...
    def action_close(self, cr, uid, ids, context=None):
        ...
        poa_programa_obj = self.pool.get('poa.programa')
        project_obj = self.pool.get('project.project')
        for this in self.browse(cr, uid, ids):
            ...
            if project_ids:
                for project_id in project_ids:
                    project = project_obj.browse(cr, uid, project_id)
                    if not project.codigo_programa in dict_project:
                        dict_project[project.codigo_programa]=project.programa
            for pr_line in dict_project:
                poa_programa_obj.create(cr, uid, {
                    'p_id':this.id,
                    'code':pr_line,
                    'name':dict_project[pr_line],
                })
    ...
    def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for this in self.browse(cr, uid, ids, context=context):
            ...
            aux_monto = 0
            for line in this.budget_ids:
                if line.type_budget=='gasto':
                    aux_monto += line.planned_amount
            res[this.id]['total_presupuesto'] = aux_monto
        return res
    ...
    def build_periods(self, cr, uid, obj):
        ...
        return period_ids
    _columns = dict(
        programa_ids = fields.one2many('poa.programa','p_id','Programas'),
        funcion_ids = fields.one2many('budget.poa.funcion','p_id','Detalle Funciones'),
        total_presupuesto = fields.function(_amount_all, string='Total Presupuesto',multi="sums",store=True),
...
class BudgetPost(osv.Model):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        for this in self.browse(cr, uid, ids):
            if vals.has_key('code') or vals.has_key('name') or vals.has_key('code_aux'):
                raise osv.except_osv('Error de usuario','No se permite editar partidas del catalogo, debe crear nuevas')
        return super(BudgetPost, self).write(cr, uid, ids, vals, context=context)    

    def create1(self, cr, uid, vals, context=None):
        #validar que no se duplique
        post_obj = self.pool.get('budget.post')
        post_ids = post_obj.search(cr, uid, [('code','=',vals['code']),('activo','=',True)])
        if len(post_ids)>0:
            raise osv.except_osv('Error de usuario','No se permite duplicar partidas. O Desactive la partida que esta con el mismo codigo')
    ...
    _columns = {
        'is_pac':fields.boolean('Reporte PAC'),
        'publicado':fields.boolean('Publicado PAC?'),
        'codigo_catalogo':fields.char('Cod. Catalogo',size=32),
        'tipo_pac':fields.selection([('Bien','Bien'),('Servicio','Servicio'),('Obra','Obra'),('Consultoria','Consultoria')],'Tipo'),
        'year_id':fields.many2one('budget.poa','Anio'),
        ...
        'name': fields.char('Partida', size=256, required=True),
...
#    _sql_constraints = [('unique_name_budget', 'unique(name)', u'El nombre de la partida es unico.')]
...
class budgetFinanciamiento(osv.Model):
    ...
    _columns = dict(
        tipo = fields.selection([('SC','Saldo Caja Bancos'),('PI','Partida Ingreso')],'Tipo'),
        ...
        if context.get('move_ids'):
            ...
            for item_id in fetch_item_ids:
                res[item_id[0]] = {
                    ...
                    'reserved_balance': 0.00, #saldo x certificar
            ...
            for poa_id in poa_ids:
                ...
                for item_id in fetch_item_ids:
                    res[item_id[0]] = {
                        ...
                        'planned_amount': item_id[1],
                        'codif_amount': 0.00,  # codificado
                        'reserved_balance': 0.00, #saldo x comprometer
                        'commited_balance': 0.00, #saldo x comprometer
                        'devengado_balance': 0.00, #saldo x devengar
                        'paid_amount': 0.00, #pagado/recaudado
                        'devengado_amount': 0.00, #devengado
                ...
                sql_certificaciones = "SELECT budget_certificate_line.id,budget_certificate_line.amount,budget_certificate_line.amount_certified,budget_certificate_line.amount_commited,budget_id FROM budget_certificate_line,budget_certificate \
                WHERE budget_certificate.state in ('commited','certified') and budget_certificate_line.certificate_id=budget_certificate.id and budget_id %s %s \
                AND budget_certificate.date_confirmed is not Null and budget_certificate.date_confirmed>='%s' and budget_certificate.date_confirmed<='%s'" % (operador,tuple_items,date_start, date_end)
                cr.execute(sql_certificaciones)
                data_certificaciones = cr.fetchall()
                tmp_cert = {}
                for req_c in data_certificaciones:
                    if req_c[2]:
                        res[req_c[4]]['reserved_amount'] = res.get(req_c[4],0)['reserved_amount'] + req_c[2]
                    else:
                        res[req_c[4]]['reserved_amount'] = res.get(req_c[4],0)['reserved_amount'] + 0
    ...
    FROM account_move_line,budget_item,budget_post,account_move WHERE account_move.id=account_move_line.move_id and account_move.state='posted' and budget_item.budget_post_id=budget_post.id and account_move_line.budget_id=budget_item.id and account_move_line.migrado=False and account_move_line.state='valid' and (budget_accrued=true or budget_paid=true) and account_move_line.budget_id %s %s and account_move_line.date>='%s' and account_move_line.date<='%s'"%(operador,tuple_items, date_start, date_end)
                ...
                for item_id in item_ids:
                    ...
                    if res[item_id]['type_budget'] == 'gasto':
                        ...
                        res[item_id]['reserved_amount'] = res[item_id]['reserved_amount']
                        res[item_id]['commited_balance'] = res[item_id]['codif_amount'] - res[item_id]['commited_amount']
                        res[item_id]['reserved_balance'] = res[item_id]['codif_amount'] - res[item_id]['reserved_amount']
                    else:
                        res[item_id]['commited_amount'] = 0
                        res[item_id]['reserved_amount'] = 0
                        res[item_id]['commited_balance'] = 0
                        res[item_id]['reserved_balance'] = 0
...
#                'paid_amount',
    ...
    _columns = {
        ...
        'reserved_balance': fields.function(_compute_budget_all, method=True, multi='budget', store=STORE_VAR,
                                            string='Saldo x Certificar', digits_compute=DP),
...
class BudgetCertificate(osv.Model):
    ...
    _columns = {
        'iva':fields.float('Total Iva',help="Este valor es unicamente informativo, se utiliza para el registro contable del IVA"),
    ...
    def action_request(self, cr, uid, ids, context=None):
        ...
        for obj in self.browse(cr, uid, ids, context):
            ...
            for line in obj.line_ids:
                ...
                #    raise osv.except_osv('Error de usuario', 'No se puede agregar items con tipo IVA en los documentos presupuestarios.')
...
#        self.__logger.info("Presupuesto referencial anulado")
    ...
    def action_draft(self, cr, uid, ids, context=None):
        ...
        self.write(cr, uid, ids, {'state': 'draft',
                                  'log_repetido':'',})
        #se regresa tambien la solicitud de pago
        if obj.payment_id:
            pay_obj.write(cr, uid, obj.payment_id.id,{
                'state':'Recibido Financiero',
            })
        return True
    ...
    def _checkCerrado(self, cr, uid, ids):
        """
        Verifica que el documento tenga asignado un proveedor
        """
        return True
        poa_obj = self.pool.get('budget.poa')
        for obj in self.browse(cr, uid, ids):
            for cert_line in obj.line_ids:
                if cert_line.budget_id.poa_id.state!='Abierto':
                    return False
        return True
    ...
    _constraints = [
        ...
        (_checkCerrado, ustr('No se puede editar documentos presupuestarios de presupuesto ya cerrado'),
         ['Documento Presupuestario', 'Detalle Partidas']),
...
class BudgetCertificateLine(osv.Model):
    ...
    def onchange_budget_disp(self, cr, uid, ids, budget_id, context={}):
        ...
        item = item_obj.browse(cr, uid, budget_id,context=context)
        total = item.commited_balance
        total2 = item.reserved_balance
        return {'value':{'budget_disponible':total,'budget_disponible_certificado':total2}}
...
#            res[obj.id] = total
    ...
    def _amount_disponible_cert(self, cr, uid, ids, fields, context, args):
        res = {}
        for obj in self.browse(cr, uid, ids, context):
#            total = obj.budget_id.commited_balance
#            res[obj.id] = total
    ...
    _columns = {
        ...
        'budget_disponible_certificado': fields.function(
            _amount_disponible_cert, method=True,
            string='Saldo Por Certificar',
            digits_compute=DP, store=False),        
