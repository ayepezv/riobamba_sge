...
class import_complemento(osv.osv_memory):

    _name='import.complemento'
...
#            result=False
    ...
    def import_complemento(self, cr, uid, ids, context=None):
        acc_obj=self.pool.get('account.account')
        ...
        if data['archivo']:
            ...
            for r in range(sh.nrows)[1:]:
                cta_id = acc_obj.search(cr, uid, [('code','=',sh.cell(r,0).value)])
                cta_comp_id=acc_obj.search(cr, uid, [('code','=',sh.cell(r,1).value)])
                if cta_id and cta_comp_id:
                    acc_obj.write(cr, uid, cta_id[0],{
                            'account_comp_id':cta_comp_id[0],
                            })
        else:
